...
use casr::pipeline::rollback_write_paths;
...
fn writer_codex_date_hierarchy_in_path() {
    ...
    // Inspect path components through the platform API: Windows uses `\\`
    // separators (and may add a `\\?\` prefix), while POSIX uses `/`.
    let components: Vec<String> = written.paths[0]
        .components()
...
#[test]
fn file_based_writers_reject_session_ids_that_escape_provider_roots() {
    let _clawdbot_lock = CLAWDBOT_ENV.lock().unwrap();
    let _vibe_lock = VIBE_ENV.lock().unwrap();
    let _factory_lock = FACTORY_ENV.lock().unwrap();
    let _openclaw_lock = OPENCLAW_ENV.lock().unwrap();

    let tmp = tempfile::TempDir::new().unwrap();
    let clawdbot_home = tmp.path().join("clawdbot");
    let vibe_home = tmp.path().join("vibe");
    let factory_home = tmp.path().join("factory");
    let openclaw_home = tmp.path().join("openclaw");
    let _clawdbot_env = EnvGuard::set("CLAWDBOT_HOME", &clawdbot_home);
    let _vibe_env = EnvGuard::set("VIBE_HOME", &vibe_home);
    let _factory_env = EnvGuard::set("FACTORY_HOME", &factory_home);
    let _openclaw_env = EnvGuard::set("OPENCLAW_HOME", &openclaw_home);

    let mut session = simple_session();
    session.session_id = "../escape".to_string();
    let opts = WriteOptions { force: false };

    assert!(
        ClawdBot.write_session(&session, &opts).is_err(),
        "ClawdBot must reject traversal IDs"
    );
    assert!(
        Vibe.write_session(&session, &opts).is_err(),
        "Vibe must reject traversal IDs"
    );
    assert!(
        Factory.write_session(&session, &opts).is_err(),
        "Factory must reject traversal IDs"
    );
    assert!(
        OpenClaw.write_session(&session, &opts).is_err(),
        "OpenClaw must reject traversal IDs"
    );

    assert!(!tmp.path().join("escape.jsonl").exists());
    assert!(!tmp.path().join("escape/messages.jsonl").exists());
}

/// A failed read-back must restore the Codex thread index together with the
/// rollout file; otherwise `codex resume` sees an orphaned native session.
#[test]
fn writer_codex_rollback_restores_thread_index_and_rollout() {
    let _lock = CODEX_ENV.lock().unwrap();
    let tmp = tempfile::TempDir::new().unwrap();
    let _env = EnvGuard::set("CODEX_HOME", tmp.path());
    ...
    assert!(
        written
            .rollback_paths
            .iter()
            .any(|entry| entry.target_path == db_path),
        "Codex rollback metadata must include the thread-index database"
    );

    rollback_write_paths("codex", &written.rollback_paths)
        .expect("rollback should restore the rollout and thread index");

    assert!(
        !written.paths[0].exists(),
        "rollback should remove the newly-created rollout"
    );
    let conn = rusqlite::Connection::open(&db_path).unwrap();
    let count: i64 = conn
        .query_row(
        ...
        .unwrap();
    assert_eq!(count, 0, "rollback must remove the registered thread row");
}
