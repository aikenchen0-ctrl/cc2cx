...
mod codex_golden {
    ...
    fn golden_codex_path_has_date_hierarchy() {
        ...
        // Verify the sessions/YYYY/MM/DD pattern exists on every platform.
        let sessions_idx = components
            .iter()
            .position(|component| component == "sessions")
            .unwrap_or_else(|| panic!("Codex path should contain sessions/, got: {path:?}"));
        assert!(
            components.len() > sessions_idx + 4,
            "Codex path should have YYYY/MM/DD/rollout- after sessions/, got: {path:?}"
        );
        let segments = &components[sessions_idx + 1..sessions_idx + 4];
        assert!(
            segments[0].len() == 4 && segments[0].chars().all(|c| c.is_ascii_digit()),
            "Codex path year should be 4 digits, got segments: {segments:?}"
        );
        assert!(
            segments[1].len() == 2 && segments[1].chars().all(|c| c.is_ascii_digit()),
            "Codex path month should be 2 digits, got segments: {segments:?}"
        );
        assert!(
            segments[2].len() == 2 && segments[2].chars().all(|c| c.is_ascii_digit()),
            "Codex path day should be 2 digits, got segments: {segments:?}"
        );
        assert!(
            components[sessions_idx + 4].starts_with("rollout-"),
            "Codex filename should start with rollout-, got: {components:?}"
        );
    }
...
mod gemini_golden {
    ...
    fn golden_gemini_path_includes_hash_and_chats() {
        ...
        let components: Vec<String> = path
            ...
            .collect();
        assert!(
            components.iter().any(|component| component == "chats"),
            "Gemini path should contain a chats directory, got: {path:?}"
        );
        // Path should include the hash directory.
        assert!(
            components.iter().any(|component| component == "tmp"),
