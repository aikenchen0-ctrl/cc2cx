...
#[test]
fn pipeline_convert_source_path_uses_explicit_provider_and_workspace() {
    let tmp = tempfile::tempdir().expect("tempdir");
    let source_path = tmp.path().join("source.json");
    fs::write(&source_path, "synthetic source").expect("write source marker");
    ...
    let dst = MockProvider::new(
        ...
        "mock-target",
        "tgt",
        vec![tmp.path().join("target")],
    );
    src.set_read_session(&source_path, valid_session_with_id("explicit-source"));
    ...
    opts.workspace_override = Some(workspace.clone());

    let result = pipeline
        .convert_source_path("src", &source_path, "tgt", opts)
        .expect("explicit source conversion should resolve");

    assert_eq!(result.source_provider, "mock-source");
    assert_eq!(result.target_provider, "mock-target");
    assert_eq!(
        result.canonical_session.workspace,
        Some(casr::pipeline::absolutize_workspace(&workspace))
    );
}

#[test]
fn pipeline_convert_source_path_rejects_missing_path_before_target_write() {
    let tmp = tempfile::tempdir().expect("tempdir");
    ...
    let dst_state = dst.clone();
    let pipeline = ConversionPipeline {
        registry: ProviderRegistry::new(vec![Box::new(src), Box::new(dst)]),
    };

    let err = pipeline
        .convert_source_path("src", &missing, "tgt", ConvertOptions::default())
        .expect_err("missing explicit source must fail");

    assert!(err.to_string().contains("not found"));
    assert_eq!(dst_state.write_calls(), 0);
}
