...
fn setup_cc_fixture_custom(
    ...
    // The fixture is JSONL. Escape a Windows workspace path before replacing
    // the JSON string value; raw backslashes would make `\c`-style invalid
    // JSON escapes and the provider would return an empty session.
    let escaped_cwd = serde_json::to_string(cwd)
        .expect("workspace path should serialize")
        .trim_matches('"')
        .to_string();
    let content = original_content
        .replace(original_session_id, &session_id)
        .replace(original_cwd, &escaped_cwd);
