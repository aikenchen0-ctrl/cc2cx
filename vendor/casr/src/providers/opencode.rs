...
use crate::pipeline::{backup_sqlite_database, rollback_after_write_error, sqlite_rollback_paths};
...
#[cfg(test)]
thread_local! {
    /// Unit-test-only database override. A thread-local keeps tests from
    /// mutating process environment while still isolating discovery from a
    /// developer's global OpenCode database.
    static TEST_DB_FILES_OVERRIDE: RefCell<Option<Vec<PathBuf>>> = const { RefCell::new(None) };
}
...
impl OpenCode {
    ...
    fn find_db_files() -> Vec<PathBuf> {
        #[cfg(test)]
        if let Some(paths) = TEST_DB_FILES_OVERRIDE.with(|slot| slot.borrow().clone()) {
            let normalized = paths
                .into_iter()
                .map(|path| path.canonicalize().unwrap_or(path))
                .collect();
            return dedup_existing_files(normalized);
        }
    ...
    fn parse_virtual_path(path: &Path) -> Option<(PathBuf, String)> {
        ...
        let is_default_database = parent.file_name().and_then(|n| n.to_str()) == Some(DB_FILENAME);
        let is_discovered_database = Self::find_db_files().into_iter().any(|known| {
            known
                .canonicalize()
                .ok()
                .zip(parent.canonicalize().ok())
                .is_some_and(|(known, parent)| known == parent)
        });
        if !is_default_database && !is_discovered_database {
            return None;
        }
...
"#,
    ...
    /// Enumerate sessions from an explicit set of database paths.
    ///
    /// Keeping the path set as an input makes enumeration deterministic for
    /// callers that already resolved their search roots and for tests that
    /// must not inspect a developer's global OpenCode database.
    fn list_sessions_from_db_files(db_files: &[PathBuf]) -> Vec<(String, PathBuf)> {
...
impl Provider for OpenCode {
    ...
    fn write_session(
        ...
        // Snapshot before opening the writer. A post-commit read-back failure
        // must be able to restore the whole SQLite database, including any WAL
        // sidecars, rather than only removing the virtual session path.
        let backup_path = backup_sqlite_database(&db_path, self.slug())?;
        let rollback_paths = sqlite_rollback_paths(&db_path, backup_path.clone());

        let result = (|| -> anyhow::Result<WrittenSession> {
            ...
            Ok(WrittenSession {
                ...
                rollback_paths: rollback_paths.clone(),
                warnings: Vec::new(),
            })
        })();
        ...
        match result {
            ...
            Err(error) => Err(rollback_after_write_error(
                self.slug(),
                error,
                &rollback_paths,
            )),
    ...
    fn write_support(&self) -> crate::providers::WriteSupport {
        crate::providers::WriteSupport::Conditional {
            reason: "OpenCode writes only to CASR-compatible legacy schemas; live 1.x/2.x databases are checked at runtime."
                .to_string(),
        }
    }
    ...
    fn list_sessions(&self) -> Option<Vec<(String, PathBuf)>> {
        ...
        Some(Self::list_sessions_from_db_files(&db_files))
    }
}
...
mod tests {
    ...
    struct CwdGuard {
        original: PathBuf,
        previous_db_files: Option<Vec<PathBuf>>,
    }
    ...
    impl CwdGuard {
        fn change_to(path: &Path) -> Self {
            ...
            let previous_db_files = super::TEST_DB_FILES_OVERRIDE
                .with(|slot| slot.replace(Some(vec![path.join(DATA_DIRNAME).join(DB_FILENAME)])));
            Self {
                original,
                previous_db_files,
            }
    ...
    impl Drop for CwdGuard {
        fn drop(&mut self) {
            ...
            super::TEST_DB_FILES_OVERRIDE.with(|slot| {
                slot.replace(self.previous_db_files.take());
            });
        }
    }
    ...
    #[test]
    fn virtual_path_accepts_custom_database_filename_from_env_override() {
        let _lock = OPENCODE_ENV.lock().expect("mutex lock");
        let tmp = tempfile::tempdir().expect("tmpdir");
        let db = tmp.path().join("custom.sqlite");
        std::fs::write(&db, "").expect("touch db file");
        let previous = TEST_DB_FILES_OVERRIDE.with(|slot| slot.replace(Some(vec![db.clone()])));

        let virtual_path = OpenCode::virtual_session_path(&db, "custom-session");
        let parsed = OpenCode::parse_virtual_path(&virtual_path);

        TEST_DB_FILES_OVERRIDE.with(|slot| {
            slot.replace(previous);
        });

        assert_eq!(
            parsed,
            Some((db, "custom-session".to_string())),
            "the configured OpenCode database may use a custom filename"
        );
    }
    ...
    #[test]
    fn write_exposes_rollback_path_for_post_commit_readback_failure() {
        let _lock = OPENCODE_ENV.lock().expect("mutex lock");
        let tmp = tempfile::tempdir().expect("tmpdir");
        let workspace = tmp.path().join("workspace");
        std::fs::create_dir_all(&workspace).expect("workspace dir");
        ...
        let seed = Connection::open(&db_path).expect("create seed database");
        seed.execute_batch("CREATE TABLE seed (value TEXT NOT NULL);")
        ...
        assert!(
            !written.rollback_paths.is_empty(),
            "a database-backed write must expose a rollback path so a post-commit read-back failure cannot leave rows behind"
        );
        crate::pipeline::rollback_write_paths("opencode", &written.rollback_paths)
            .expect("rollback should remove the converted session");

        let conn = OpenCode::open_db(&db_path).expect("database should remain readable");
        assert!(
            !OpenCode::session_exists(&conn, DbSchema::Legacy, &written.session_id),
            "rollback should remove the written session and dependent messages"
        );
    }
...
"#,
    ...
    fn list_sessions_empty_db_returns_empty_vec() {
        ...
        // Enumerate only the synthetic DB so a developer's global OpenCode
        // history cannot make the empty-database assertion flaky.
        let listed = OpenCode::list_sessions_from_db_files(std::slice::from_ref(&db_path));
