...
impl Provider for Kiro {
    ...
    fn write_session(
        ...
        written_paths.push(json_outcome.target_path.clone());
        rollback_paths.push(crate::providers::RollbackPath::new(
            json_outcome.target_path,
            json_outcome.backup_path,
        ));

        let jsonl_outcome = match crate::pipeline::atomic_write(
            &jsonl_path,
            jsonl.as_bytes(),
            opts.force,
            self.slug(),
        ...
        written_paths.push(jsonl_outcome.target_path.clone());
        rollback_paths.push(crate::providers::RollbackPath::new(
            jsonl_outcome.target_path,
            jsonl_outcome.backup_path,
        ));
        ...
        if let Some(history) = session.metadata.get("history").and_then(|v| v.as_str()) {
            let hist_outcome = match crate::pipeline::atomic_write(
                &history_path,
                history.as_bytes(),
                opts.force,
                self.slug(),
            ...
            written_paths.push(hist_outcome.target_path.clone());
            rollback_paths.push(crate::providers::RollbackPath::new(
                hist_outcome.target_path,
                hist_outcome.backup_path,
            ));
        }
