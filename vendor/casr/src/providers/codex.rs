...
impl Provider for Codex {
    ...
    fn write_session(
        ...
        let (warnings, thread_registration) = warnings;
        let mut rollback_paths = vec![crate::providers::RollbackPath::new(
            outcome.target_path.clone(),
            outcome.backup_path.clone(),
        )];
        let backup_path = if let Some(registration) = thread_registration {
            rollback_paths.extend(registration.rollback_paths);
            Some(registration.backup_path)
        } else {
            outcome.backup_path.clone()
        };
...
impl Codex {
    ...
    fn register_thread(
        ...
        preview: &str,
        now: &chrono::DateTime<chrono::Utc>,
    ) -> (Vec<String>, Option<ThreadRegistration>) {
        ...
        let backup_path = match crate::pipeline::backup_sqlite_database(&db_path, "codex") {
            Ok(Some(path)) => path,
            Ok(None) => {
                return (
                    vec![format!(
                        "Codex thread index {} disappeared before registration; the rollout \
                         file was written to {}.",
                        db_path.display(),
                        rollout_path.display()
                    )],
            ...
            Err(error) => {
                return (
                    vec![format!(
                        "Could not snapshot the Codex thread index ({db}): {error}; \
                         the rollout file was written to {path}.",
                        db = db_path.display(),
        ...
        match register_thread_in_db(
            ...
            Ok(()) => {
                ...
                (
                    ...
                    Some(ThreadRegistration {
                        rollback_paths: crate::pipeline::sqlite_rollback_paths(
                            &db_path,
            ...
            Err(e) => {
                ...
                let rollback_paths =
                    crate::pipeline::sqlite_rollback_paths(&db_path, Some(backup_path.clone()));
                let rollback_error =
                    crate::pipeline::rollback_write_paths("codex", &rollback_paths).err();
                let cleanup = rollback_error
                    .as_ref()
                    .map(|error| format!(" The index snapshot could not be restored: {error}."))
                    .unwrap_or_default();
                (
                    vec![format!(
                        "Could not register the session in the Codex thread index \
                         ({db}): {e}.{cleanup} `codex resume {session_id}` may report \
...
struct ThreadRegistration {
    backup_path: PathBuf,
    rollback_paths: Vec<crate::providers::RollbackPath>,
}
