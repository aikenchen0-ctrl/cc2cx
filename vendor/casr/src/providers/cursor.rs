...
#[cfg(test)]
thread_local! {
    /// Test-only config directory override. A thread-local keeps Cursor unit
    /// tests isolated without mutating the process environment (which is
    /// unsafe under Rust 2024's concurrent test harness).
    static TEST_CONFIG_DIR_OVERRIDE: RefCell<Option<PathBuf>> = const { RefCell::new(None) };
}
...
impl Cursor {
    ...
    fn config_dir() -> Option<PathBuf> {
        #[cfg(test)]
        if let Some(path) = TEST_CONFIG_DIR_OVERRIDE.with(|slot| slot.borrow().clone()) {
            return Some(path);
        }
...
impl Provider for Cursor {
    ...
    fn write_session(
        ...
        // must be able to restore the whole SQLite database, not just remove
        // the virtual session path returned below.
        let backup_path = backup_sqlite_database(&global_db, self.slug())?;
        let rollback_paths = sqlite_rollback_paths(&global_db, backup_path.clone());
...
mod tests {
    ...
    impl ConfigDirGuard {
        fn set(value: &Path) -> Self {
            let previous =
                TEST_CONFIG_DIR_OVERRIDE.with(|slot| slot.replace(Some(value.to_path_buf())));
            Self { previous }
        }
    }
    ...
    impl Drop for ConfigDirGuard {
        fn drop(&mut self) {
            TEST_CONFIG_DIR_OVERRIDE.with(|slot| {
    ...
    fn write_exposes_rollback_path_for_post_commit_readback_failure() {
        ...
        let db_path = tmp.path().join("User/globalStorage/state.vscdb");
        std::fs::create_dir_all(db_path.parent().expect("database parent"))
            .expect("create database parent");
        let seed = Cursor::open_db_rw(&db_path).expect("create seed database");
        seed.execute_batch(
            "CREATE TABLE IF NOT EXISTS cursorDiskKV (key TEXT PRIMARY KEY, value TEXT);",
        )
        ...
        let source = CanonicalSession {
            session_id: "rollback-source".to_string(),
            provider_slug: "claude-code".to_string(),
            workspace: None,
            ...
            metadata: json!({}),
            source_path: tmp.path().join("source.jsonl"),
            model_name: None,
        };
        ...
        crate::pipeline::rollback_write_paths("cursor", &written.rollback_paths)
            .expect("rollback should remove the converted session");

        let conn = Cursor::open_db(&db_path).expect("database should remain readable");
        assert!(
            !Cursor::list_composer_ids(&conn).contains(&written.session_id),
            "rollback should remove the written composer and its bubbles"
        );
    }
