...
impl Cline {
    ...
    fn update_task_history(
        ...
        provider_slug: &str,
    ) -> anyhow::Result<crate::pipeline::AtomicWriteOutcome> {
        let history_path = Self::task_history_path(storage_root);
...
impl Provider for Cline {
    ...
    fn write_session(
        ...
        rollback_paths.push(crate::providers::RollbackPath::new(
            api_outcome.target_path,
            api_outcome.backup_path,
        ));
        ...
        let ui_outcome =
            match crate::pipeline::atomic_write(&ui_path, &ui_bytes, opts.force, self.slug()) {
                Ok(outcome) => outcome,
                Err(error) => {
        ...
        rollback_paths.push(crate::providers::RollbackPath::new(
            ui_outcome.target_path,
            ui_outcome.backup_path,
        ));
        ...
        rollback_paths.push(crate::providers::RollbackPath::new(
            metadata_outcome.target_path,
            metadata_outcome.backup_path,
        ));

        // 4) state/taskHistory.json (best-effort, but needed for Cline to list tasks)
        let history_outcome =
            match Self::update_task_history(&storage_root, &target_task_id, session, self.slug()) {
                Ok(outcome) => outcome,
                Err(error) => {
                    return Err(crate::pipeline::rollback_after_write_error(
                        self.slug(),
        ...
        let backup_path = history_outcome.backup_path.clone();
        rollback_paths.push(crate::providers::RollbackPath::new(
            history_outcome.target_path,
            history_outcome.backup_path,
        ));
