...
/// Return a Pi-Agent-safe filename stem for a canonical session ID.
///
/// Canonical IDs can originate in another provider, so they must not be used
/// as path fragments without validation. Pi-Agent stores the stem directly
/// below its sessions directory and requires an underscore in the name.
fn safe_session_id(raw: &str) -> anyhow::Result<String> {
    if !raw.is_empty()
        && (raw == "."
            || raw == ".."
            || raw.len() > 200
            || raw.chars().last().is_some_and(|ch| ch == '.' || ch == ' ')
            || raw.chars().any(|ch| {
                ch.is_control()
                    || matches!(ch, '<' | '>' | ':' | '"' | '/' | '\\' | '|' | '?' | '*')
            }))
    {
        anyhow::bail!("invalid session id for Pi-Agent");
    }
    ...
    let candidate = if raw.is_empty() {
        ...
        let now = chrono::Utc::now();
        format!("{}_{}", now.format("%Y-%m-%dT%H-%M-%S"), raw)
    };
...
impl Provider for PiAgent {
    ...
    fn write_session(
        ...
        let session_id = safe_session_id(&session.session_id)?;
...
mod tests {
    ...
    #[test]
    fn writer_rejects_session_ids_that_escape_the_sessions_directory() {
        for session_id in [
            "../escape",
            r"..\escape",
            "C:\\escape",
            ...
            let error =
                safe_session_id(session_id).expect_err("unsafe session id must be rejected");
    ...
    fn writer_resume_command() {
        ...
        let path = cmd
            ...
            .map(Path::new)
            .expect("resume command should use the Pi session flag");
        assert_eq!(
            path.file_name().and_then(|name| name.to_str()),
            Some("my-session.jsonl")
        );
        assert_eq!(
            path.parent()
