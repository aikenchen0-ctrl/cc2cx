...
// Aider keeps every session for a workspace in one append-only Markdown file.
// Serialize read/append/replace operations within this process so concurrent
// transfers cannot both snapshot the same old file and drop one another's
// session.
static AIDER_WRITE_LOCK: OnceLock<Mutex<()>> = OnceLock::new();

const AIDER_LOCK_WAIT: std::time::Duration = std::time::Duration::from_secs(30);
const AIDER_LOCK_STALE_AFTER: std::time::Duration = std::time::Duration::from_secs(300);

struct AiderHistoryLock {
    path: PathBuf,
    _file: std::fs::File,
}

impl Drop for AiderHistoryLock {
    fn drop(&mut self) {
        let _ = std::fs::remove_file(&self.path);
    }
}

fn aider_history_lock_path(target_path: &Path) -> PathBuf {
    let file_name = target_path
        .file_name()
        .and_then(|name| name.to_str())
        .unwrap_or("aider-history");
    target_path.with_file_name(format!(".{file_name}.casr.lock"))
}

/// Acquire a bounded cross-process lock for the shared Aider history file.
///
/// The in-process mutex below handles normal cc2cx concurrency. This lock file
/// closes the gap for two independent CASR processes, while an age-based
/// reclaim prevents a crashed process from blocking future transfers forever.
fn acquire_history_lock(target_path: &Path) -> anyhow::Result<AiderHistoryLock> {
    ...
    let lock_path = aider_history_lock_path(target_path);
    let deadline = std::time::Instant::now() + AIDER_LOCK_WAIT;
            ...
            .open(&lock_path)
        {
            ...
            Err(error) if error.kind() == std::io::ErrorKind::AlreadyExists => {
                let stale = std::fs::metadata(&lock_path)
                    ...
                    .is_ok_and(|age| age > AIDER_LOCK_STALE_AFTER);
                if stale {
                    match std::fs::remove_file(&lock_path) {
                        ...
                        Err(remove_error) => {
                            return Err(anyhow::anyhow!(
                                "failed to reclaim stale Aider history lock {}: {remove_error}",
                                lock_path.display()
                            ));
                        }
                    }
                ...
                if std::time::Instant::now() >= deadline {
                    anyhow::bail!(
                        "timed out waiting for Aider history lock {}",
                        lock_path.display()
                    );
                }
            ...
            Err(error) => {
                return Err(anyhow::anyhow!(
                    "failed to acquire Aider history lock {}: {error}",
                    lock_path.display()
                ));
            }
        }
...
impl Provider for Aider {
    ...
    fn write_session(
        ...
        // The history file is shared by all sessions in a workspace. Keep the
        // read/append/replace sequence together so two in-process transfers
        // cannot overwrite one another with stale snapshots.
        let _write_guard = AIDER_WRITE_LOCK
            .get_or_init(|| Mutex::new(()))
            .lock()
            .map_err(|_| anyhow::anyhow!("Aider history write lock was poisoned"))?;
        let _process_lock = acquire_history_lock(&target_path)?;

        let existing_content = if target_path.is_file() {
            std::fs::read_to_string(&target_path)
                .with_context(|| format!("failed to read {}", target_path.display()))?
        } else {
            String::new()
        };
        let existing_ids: HashSet<String> = Self::split_sessions(&existing_content)
            .into_iter()
            .map(|parsed| parsed.session_id)
            .collect();
        let start = unique_session_start_at(chrono::Utc::now(), &existing_ids);
        let target_session_id = start.format("%Y-%m-%dT%H-%M-%S").to_string();
        let start_timestamp = start.format("%Y-%m-%d %H:%M:%S").to_string();
        ...
        // Preserve all previously written sessions in the shared history.
        ...
        output.push_str(&format!("\n# aider chat started at {start_timestamp}\n\n"));
        ...
        let rollback_target = outcome.target_path.clone();
        let rollback_backup = outcome.backup_path.clone();
        ...
        Ok(WrittenSession {
            ...
            backup_path: rollback_backup.clone(),
            rollback_paths: vec![crate::providers::RollbackPath::new(
                rollback_target,
                rollback_backup,
            )],
            warnings: Vec::new(),
...
/// Pick the first whole-second timestamp that is not already represented in
/// the shared Aider history file. Aider's native session ID is derived from
/// this timestamp, so keeping the original header format preserves native
/// compatibility while avoiding same-second ID collisions.
fn unique_session_start_at(
    now: chrono::DateTime<chrono::Utc>,
    existing_ids: &HashSet<String>,
) -> chrono::DateTime<chrono::Utc> {
    let mut candidate = now;
    loop {
        let id = candidate.format("%Y-%m-%dT%H-%M-%S").to_string();
...
#[cfg(test)]
fn unique_session_id_at(
    now: chrono::DateTime<chrono::Utc>,
    existing_ids: &HashSet<String>,
) -> String {
    unique_session_start_at(now, existing_ids)
        .format("%Y-%m-%dT%H-%M-%S")
        .to_string()
}
...
mod tests {
    ...
    #[test]
    fn unique_session_id_advances_when_second_is_already_used() {
        let now = chrono::Utc
            .with_ymd_and_hms(2024, 8, 5, 19, 33, 2)
            .single()
            .expect("valid timestamp");
        let existing = std::collections::HashSet::from(["2024-08-05T19-33-02".to_string()]);

        let selected = unique_session_id_at(now, &existing);

        assert_eq!(selected, "2024-08-05T19-33-03");
    }

    #[test]
    fn history_lock_waits_for_another_writer_to_release() {
        let tmp = tempfile::tempdir().expect("tempdir");
        let target = tmp.path().join(".aider.chat.history.md");
        let first = acquire_history_lock(&target).expect("first writer should acquire lock");
        ...
        let second =
            acquire_history_lock(&target).expect("second writer should eventually acquire lock");
        assert!(
            started.elapsed() >= std::time::Duration::from_millis(25),
            "second writer did not wait for the existing lock"
        );
        drop(second);
        release.join().expect("lock owner thread should exit");
    }
