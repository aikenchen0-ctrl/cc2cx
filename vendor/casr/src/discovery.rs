...
use crate::providers::{Provider, ProviderCapability};
...
/// A session discovered from one provider's native storage.
#[derive(Debug, Clone)]
...
impl ProviderRegistry {
    ...
    /// Return capability metadata for every registered provider.
    ///
    /// This intentionally includes providers that are not installed so a UI
    /// can explain why a target is unavailable instead of hiding it.
    pub fn capabilities(&self) -> Vec<ProviderCapability> {
        self.providers
            .iter()
            .map(|provider| ProviderCapability {
                slug: provider.slug().to_string(),
                alias: provider.cli_alias().to_string(),
                name: provider.name().to_string(),
                detection: provider.detect(),
                write_support: provider.write_support(),
            })
            .collect()
    }

    /// Discover readable sessions across all registered providers.
    ///
    /// Provider-specific `list_sessions()` implementations are preferred. For
    /// providers without one, a bounded walk over `session_roots()` is used.
    /// A malformed candidate is skipped; discovery never emits its contents or
    /// error text, keeping this method suitable for a user-facing session list.
    pub fn discover_sessions(&self) -> Vec<DiscoveredSession> {
        let mut discovered = Vec::new();

        for provider in &self.providers {
            let listed = provider.list_sessions();
            let candidates =
                listed.unwrap_or_else(|| discover_candidates_from_roots(provider.as_ref()));
            let mut seen = std::collections::HashSet::new();

            for (_hint, path) in candidates {
                if !is_readable_session_path(&path) {
                    continue;
                }
                ...
                let Ok(session) = provider.read_session(&path) else {
                    continue;
                };
                ...
                discovered.push(DiscoveredSession {
                    ...
                    provider_alias: provider.cli_alias().to_string(),
                    path,
                    session,
                });
            }
    ...
    fn resolve_from_path(
        ...
        debug!(
            phase = "resolve_explicit_path",
            "resolving session from explicit path"
        );
    ...
    fn resolve_with_alias(
        ...
        debug!(
            provider = alias,
            phase = "resolve_alias",
            "resolving session with source alias hint"
        );
    ...
    fn resolve_auto(&self, session_id: &str) -> Result<ResolvedSession<'_>, CasrError> {
        debug!(
            phase = "resolve_auto",
            "auto-resolving session across all providers"
        );
        ...
        match matches.len() {
            0 => {
                debug!(
                    phase = "resolve_auto",
                    provider_count = providers_checked.len(),
    ...
    /// Resolve a source selected by a caller that already knows its provider.
    ///
    /// Unlike [`resolve_session`], this does not infer a provider from file
    /// signatures and does not require the path to live below a configured
    /// session root. That is important for UI-selected archives and provider
    /// virtual paths such as `<database>/<session-id>`.
    pub fn resolve_source_path(
        ...
        let provider =
            self.find_by_alias(source_alias)
                .ok_or_else(|| CasrError::UnknownProviderAlias {
...
fn is_readable_session_path(path: &Path) -> bool {
    path.is_file() || path.is_dir() || path.parent().is_some_and(Path::is_file)
}

fn session_path_key(path: &Path) -> String {
    // Virtual database paths have a real database parent. Canonicalizing the
    // parent removes duplicate spellings without requiring the virtual leaf to
    // exist as a filesystem entry.
    if let Some(parent) = path.parent().filter(|p| p.is_file())
        && let Ok(parent) = parent.canonicalize()
    {
        let leaf = path.file_name().map(|name| name.to_string_lossy());
        return format!("{}#{}", parent.display(), leaf.unwrap_or_default());
    }
...
fn discover_candidates_from_roots(provider: &dyn Provider) -> Vec<(String, PathBuf)> {
    ...
    for root in provider.session_roots() {
        ...
        for entry in walker {
            ...
            candidates.push((String::new(), path));
        }
    }
    candidates
}
...
mod tests {
    ...
    fn tracing_macros_do_not_contain_sensitive_fields() {
        ...
        for line in source.lines() {
            ...
            if in_macro {
                for forbidden in ["path = %", "session_id,", "session_id =", "?roots"] {
                    assert!(
    ...
    impl Provider for FixtureProvider {
        ...
        fn read_session(&self, path: &Path) -> Result<CanonicalSession> {
            ...
            if content != "valid" {
                anyhow::bail!("fixture parse failure")
            }
            Ok(CanonicalSession {
                session_id: path.file_stem().unwrap().to_string_lossy().to_string(),
                ...
                messages: vec![
                    CanonicalMessage {
                        ...
                        content: "synthetic user".to_string(),
                        timestamp: None,
                        author: None,
                        tool_calls: vec![],
                        tool_results: vec![],
                    ...
                    CanonicalMessage {
                        ...
                        content: "synthetic assistant".to_string(),
                        timestamp: None,
                        author: None,
                        tool_calls: vec![],
                        tool_results: vec![],
        ...
        fn write_session(
            ...
            anyhow::bail!("fixture is read-only in this test")
        }

        fn resume_command(&self, session_id: &str) -> String {
            format!("fix --resume {session_id}")
        }

        fn list_sessions(&self) -> Option<Vec<(String, PathBuf)>> {
            Some(vec![
                ("valid".to_string(), self.root.join("valid.jsonl")),
                ("invalid".to_string(), self.root.join("invalid.jsonl")),
            ])
        }
    }

    #[test]
    fn capabilities_and_discovery_cover_listed_sessions_without_leaking_content() {
        let tmp = tempfile::tempdir().expect("tempdir");
        std::fs::write(tmp.path().join("valid.jsonl"), "valid").expect("write valid fixture");
        std::fs::write(tmp.path().join("invalid.jsonl"), "contains secret prompt")
            .expect("write invalid fixture");

        let registry = ProviderRegistry::new(vec![Box::new(FixtureProvider {
            root: tmp.path().to_path_buf(),
        })]);

        let capabilities = registry.capabilities();
        assert_eq!(capabilities.len(), 1);
        assert_eq!(capabilities[0].slug, "fixture-agent");
        assert_eq!(capabilities[0].alias, "fix");
        assert!(matches!(
            capabilities[0].write_support,
            crate::providers::WriteSupport::Supported
        ));

        let discovered = registry.discover_sessions();
        assert_eq!(discovered.len(), 1);
        assert_eq!(discovered[0].session.session_id, "valid");
        assert_eq!(discovered[0].provider_alias, "fix");
        let debug = format!("{discovered:?}");
        assert!(!debug.contains("secret prompt"));
    }

    #[test]
    fn default_registry_exposes_all_seventeen_providers_and_write_boundaries() {
        let registry = ProviderRegistry::default_registry();
        let capabilities = registry.capabilities();

        assert_eq!(capabilities.len(), 17);
        assert!(capabilities.iter().any(|cap| cap.slug == "antigravity"
            && matches!(
                cap.write_support,
                crate::providers::WriteSupport::ReadOnly { .. }
            )));
        assert!(capabilities.iter().any(|cap| cap.slug == "opencode"
            && matches!(
                cap.write_support,
                crate::providers::WriteSupport::Conditional { .. }
            )));
    }
    ...
    fn git_marker_file_with_absolute_gitdir() {
        ...
        match result {
            Some(GitMarker::File { gitdir }) => {
                let normalized = gitdir.to_string_lossy().replace('\\', "/");
                assert!(gitdir.is_absolute());
                assert!(
                    normalized.ends_with("/absolute/path/to/.git"),
                    "unexpected absolute gitdir: {}",
                    gitdir.display()
                );
            }
            other => panic!("expected GitMarker::File, got {other:?}"),
    ...
    fn git_marker_file_skips_blank_and_comment_lines() {
        ...
        match result {
            Some(GitMarker::File { gitdir }) => {
                ...
                assert!(gitdir.is_absolute());
                assert!(
                    normalized.ends_with("/some/path"),
                    "unexpected absolute gitdir: {}",
                    gitdir.display()
                );
            }
