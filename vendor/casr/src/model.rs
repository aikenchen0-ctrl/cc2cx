...
pub fn effective_workspace(session: &CanonicalSession) -> std::path::PathBuf {
    let workspace = session.workspace.clone().unwrap_or_else(|| {
        std::env::current_dir().unwrap_or_else(|_| std::path::PathBuf::from("."))
    });
    normalize_workspace_for_cli(&workspace)
}

/// Convert Windows verbatim paths to the ordinary paths expected by CLIs.
///
/// Rust's Windows `canonicalize` commonly returns `\\?\C:\...`. That form is
/// valid for Win32 file APIs but many CLI tools use the textual path to derive
/// project/session buckets and do not normalize the verbatim prefix. Keeping
/// the prefix would therefore create a bucket that the CLI cannot find when
/// launched from the same ordinary `C:\...` directory.
pub fn normalize_workspace_for_cli(path: &Path) -> PathBuf {
    #[cfg(windows)]
    {
...
mod tests {
    ...
    #[cfg(windows)]
    #[test]
    fn normalize_workspace_for_cli_strips_windows_verbatim_prefix() {
        let path = Path::new(r"\\?\C:\codeDev\_superAppAndroid_remote");

        assert_eq!(
            normalize_workspace_for_cli(path),
            PathBuf::from(r"C:\codeDev\_superAppAndroid_remote")
        );
    }
