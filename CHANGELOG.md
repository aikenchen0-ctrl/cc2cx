...
- **CC Switch schema 18 兼容**：一键迁移旧 CC Switch 数据库现在支持 `user_version=18`，幂等补齐 `session_log_sync` 的字节游标与尾部指纹列；已有供应商、MCP、Skills、会话用量和备份数据保持不变。
...
- **CC Switch SQL 自动发现**：打开 Agent 安装页时自动扫描旧 CC Switch 数据目录、备份目录和常见下载目录，只展示带 CC Switch 导出标记的 SQL，并默认选中最近修改的文件；仍可手动选择其他 SQL 文件。
...
- **macOS Agent 安装权限引导**：Codex UI 和 WorkBuddy 安装优先使用 `/Applications`，无写入权限时自动回退到 `~/Applications`；安装页面现在会展示实际安装路径，并在检测到权限错误时给出 Finder 权限检查和重试指引。
- **macOS 桌面应用状态检测**：只有包含 `Contents/Info.plist` 和 `Contents/MacOS` 可执行文件的完整应用包才会标记为已安装，启动时直接使用检测到的应用路径。
