...
import { agentInstallApi } from "@/lib/api/agentInstall";
import {
...
function App() {
  ...
  useEffect(() => {
    if (localStorage.getItem(AGENT_FIRST_LAUNCH_STORAGE_KEY)) return;
    ...
    void agentInstallApi
      ...
      .then((statuses) => {
        if (!active) return;
        localStorage.setItem(AGENT_FIRST_LAUNCH_STORAGE_KEY, "1");
        if (shouldOpenAgentInstallOnFirstLaunch(statuses, false)) {
          setIsAgentInstallOpen(true);
        }
      })
      .catch((error) => {
        console.error(
          "[App] Failed to check agent installation on startup:",
          error,
        );
      });
  ...
  return (
    ...
    >
                ...
                {currentView === "providers" && (
                  <>
                    ...
                    <Button
                      variant="ghost"
                      size="sm"
                      onClick={() => setIsAgentInstallOpen(true)}
      ...
      <AgentInstallPanel
        isOpen={isAgentInstallOpen}
        onClose={() => setIsAgentInstallOpen(false)}
      />
