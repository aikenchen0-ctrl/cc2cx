//! Panic Hook 模块
//!
//! 在应用崩溃时捕获 panic 信息并记录到 `<app_config_dir>/crash.log` 文件中（默认 `~/.cc-launch/crash.log`）。
//! 便于用户和开发者诊断闪退问题。

use crate::config::get_app_config_dir as configured_app_config_dir;
...
mod tests {
    ...
    fn test_crash_log_path() {
        ...
        assert!(path.to_string_lossy().contains(".cc-launch"));
    }
