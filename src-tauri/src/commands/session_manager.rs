...
/// 在用户选定的终端里恢复一个会话。
///
/// `command` 仍保留用于兼容旧版本 UI，但在进入任何终端启动器前必须通过
/// `validate_resume_command`。新功能应优先使用结构化的 provider/session 启动命令，
/// 避免把未经验证的文本交给 shell。
#[tauri::command]
...
#[tauri::command]
pub async fn list_session_transfer_targets() -> Result<
    Vec<crate::session_transfer::SessionTransferTarget>,
    crate::session_transfer::SessionTransferError,
> {
    crate::session_transfer::list_session_transfer_targets().await
}

#[tauri::command]
pub async fn transfer_session(
    request: crate::session_transfer::SessionTransferRequest,
) -> Result<
    crate::session_transfer::SessionTransferResult,
    crate::session_transfer::SessionTransferError,
> {
    crate::session_transfer::transfer_session(request).await
}

#[tauri::command]
pub async fn launch_transferred_session(
    request: crate::session_transfer::LaunchTransferredSessionRequest,
) -> Result<
    crate::session_transfer::LaunchTransferredSessionResult,
    crate::session_transfer::SessionTransferError,
> {
    crate::session_transfer::launch_transferred_session(request).await
}
