...
fn agent_install_specs() -> [AgentInstallSpec; 11] {
    [
        AgentInstallSpec {
            id: "codex-ui",
            name: "Codex UI",
            description: "OpenAI 的桌面 Agent 应用",
            tool: None,
            desktop: true,
            supported: true,
        ...
        AgentInstallSpec {
            id: "claude",
            name: "Claude Code",
            description: "Anthropic 的命令行编程 Agent",
            tool: Some("claude"),
            desktop: false,
            supported: true,
            unsupported_reason: None,
        ...
        AgentInstallSpec {
            id: "claude-desktop",
            name: "Claude Desktop",
            description: "Anthropic 桌面应用",
            tool: None,
            desktop: false,
            supported: false,
            unsupported_reason: Some("请通过 Anthropic 官方安装器安装 Claude Desktop"),
        },
        AgentInstallSpec {
            id: "codex",
            name: "Codex",
            description: "OpenAI 的命令行编程 Agent",
            tool: Some("codex"),
            desktop: false,
            supported: true,
            unsupported_reason: None,
        ...
        AgentInstallSpec {
            id: "gemini",
            name: "Gemini CLI",
            description: "Google Gemini 命令行 Agent",
            tool: Some("gemini"),
            desktop: false,
            supported: true,
            unsupported_reason: None,
        ...
        AgentInstallSpec {
            id: "openclaw",
            name: "OpenClaw",
            description: "OpenClaw 命令行 Agent",
            tool: Some("openclaw"),
            desktop: false,
            supported: true,
            unsupported_reason: None,
        ...
        AgentInstallSpec {
            id: "hermes",
            name: "Hermes",
            description: "Nous Research 的 Hermes Agent",
            tool: Some("hermes"),
            desktop: false,
            supported: true,
            unsupported_reason: None,
        ...
        AgentInstallSpec {
            id: "workbuddy",
            name: "WorkBuddy",
            description: "腾讯 WorkBuddy 桌面 Agent",
            tool: None,
            desktop: true,
            supported: true,
            unsupported_reason: None,
...
struct ChatGptCask {
    ...
    sha256: String,
    #[serde(default)]
    variations: HashMap<String, ChatGptCaskVariation>,
}
...
fn command_exists(command: &str) -> bool {
    ...
    process.arg(command);
    #[cfg(not(target_os = "windows"))]
    process.arg("-c").arg(format!("command -v {command}"));
...
fn parse_wsl_distribution_output(output: &str) -> Option<String> {
    output
        .lines()
        .map(str::trim)
...
fn npm_prefix_is_usable(prefix_exists: bool, write_probe_succeeded: bool) -> bool {
    prefix_exists && write_probe_succeeded
}

fn has_dotnet_runtime(output: &str) -> bool {
    output.lines().any(|line| {
        let line = line.trim();
        line.starts_with("Microsoft.WindowsDesktop.App ")
            || line.starts_with("Microsoft.NETCore.App ")
    })
}
...
fn windows_git_bash_available() -> bool {
    ...
    candidates.into_iter().any(|path| path.is_file()) || command_exists("bash")
}
...
fn windows_wsl_distribution_available() -> bool {
    ...
    output.status.success()
        && parse_wsl_distribution_output(&decode_command_output(&output.stdout)).is_some()
}
...
fn npm_global_prefix_status() -> (bool, String) {
    ...
    let Ok(output) = command.args(["config", "get", "prefix"]).output() else {
        return (false, "未找到 npm 命令".to_string());
    };
    if !output.status.success() {
        return (false, "无法读取 npm 全局目录".to_string());
    }
    let prefix = decode_command_output(&output.stdout).trim().to_string();
    if prefix.is_empty() || prefix.eq_ignore_ascii_case("undefined") {
        return (false, "npm 未返回全局目录".to_string());
    }
    let prefix_path = PathBuf::from(&prefix);
    let prefix_exists = prefix_path.is_dir() || std::fs::create_dir_all(&prefix_path).is_ok();
    if !prefix_exists {
        return (false, format!("npm 全局目录不可用: {prefix}"));
    }
    let probe_path = prefix_path.join(format!(".cc-launch-write-test-{}", uuid::Uuid::new_v4()));
    let write_probe_succeeded = std::fs::write(&probe_path, b"cc-launch").is_ok();
    let _ = std::fs::remove_file(&probe_path);
    (
        npm_prefix_is_usable(prefix_exists, write_probe_succeeded),
        if write_probe_succeeded {
            format!("npm 全局目录可写: {prefix}")
        } else {
            format!("npm 全局目录不可写: {prefix}")
        },
    )
}
...
fn windows_webview2_available() -> bool {
    ...
    locations.iter().any(|(root, path)| {
        RegKey::predef(*root)
            .open_subkey(path)
            .and_then(|key| key.get_value::<String, &str>("pv"))
            .map(|version| !version.trim().is_empty() && version.trim() != "0.0.0.0")
            .unwrap_or(false)
    })
}
...
fn windows_dotnet_runtime_available() -> bool {
    ...
    output.status.success() && has_dotnet_runtime(&decode_command_output(&output.stdout))
}
...
fn node_runtime_version() -> Option<String> {
    ...
    if let Ok(output) = process.arg("--version").output() {
        if output.status.success() {
    ...
    for directory in node_runtime_bin_dirs() {
...
fn node_runtime_bin_dirs() -> Vec<PathBuf> {
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        if let Ok(local_app_data) = std::env::var("LOCALAPPDATA") {
            ...
            directories.push(PathBuf::from(local_app_data).join("cc-launch\node"));
        }
    }

    #[cfg(not(target_os = "windows"))]
    if let Ok(home) = std::env::var("HOME") {
        directories.push(PathBuf::from(&home).join(".cc-launch/node/bin"));
...
fn node_runtime_meets_requirement() -> Option<String> {
    ...
    (major >= 18 && npm_available).then_some(version)
}

fn workbuddy_platform_identifier() -> Option<&'static str> {
    #[cfg(target_os = "windows")]
    return Some("workbuddy-win32-x64-user");
    #[cfg(all(target_os = "macos", target_arch = "aarch64"))]
    return Some("workbuddy-darwin-arm64");
    #[cfg(all(target_os = "macos", target_arch = "x86_64"))]
    return Some("workbuddy-darwin-x64");
    #[cfg(not(any(target_os = "windows", target_os = "macos")))]
    None
}

fn workbuddy_expected_hash(metadata: &WorkBuddyUpdate) -> &str {
    let sha256hash = metadata.sha256hash.trim();
    if !sha256hash.is_empty() {
        sha256hash
    } else {
        metadata.hash.trim()
    }
...
fn sha256_matches(bytes: &[u8], expected: &str) -> bool {
    !expected.trim().is_empty() && sha256_hex(bytes).eq_ignore_ascii_case(expected.trim())
}

#[cfg_attr(not(target_os = "macos"), allow(dead_code))]
fn select_chatgpt_download(cask: &ChatGptCask) -> Option<(String, String)> {
    if cfg!(target_arch = "x86_64") {
        if let Some(variation) = cask.variations.values().find(|variation| {
            variation
                .url
                .as_deref()
                .is_some_and(|url| url.contains("darwin-x64"))
        }) {
            if let (Some(url), Some(sha256)) = (&variation.url, &variation.sha256) {
                return Some((url.clone(), sha256.clone()));
            }
        }
    }
    Some((cask.url.clone(), cask.sha256.clone()))
}
    ...
    F: FnMut(DownloadProgress),
{
    ...
    let mut file = tokio::fs::File::create(destination)
        .await
        .map_err(|error| format!("创建安装包临时文件失败: {error}"))?;
    ...
    F: FnMut(DownloadProgress),
{
    ...
    let bytes = tokio::fs::read(destination)
        .await
        .map_err(|error| format!("读取安装包校验内容失败: {error}"))?;
    let digest = format!("{:x}", Sha256::digest(&bytes));
    if expected_sha256.trim().is_empty() || !digest.eq_ignore_ascii_case(expected_sha256.trim()) {
        let _ = tokio::fs::remove_file(destination).await;
        return Err("安装包 SHA-256 校验失败，已停止安装".to_string());
    }
    Ok(())
}

#[cfg(target_os = "windows")]
fn windows_signer_subject_is_trusted(subject: &str) -> bool {
    subject.to_ascii_lowercase().contains("tencent")
}
...
fn verify_windows_authenticode(path: &Path) -> Result<(), String> {
    ...
    command
        ...
        .env("CC_LAUNCH_INSTALLER_PATH", path)
        .creation_flags(CREATE_NO_WINDOW);
    let output = command
        .output()
        .map_err(|error| format!("验证 WorkBuddy 数字签名失败: {error}"))?;
    if !output.status.success() {
        let detail = decode_command_output(&output.stderr).trim().to_string();
        return Err(if detail.is_empty() {
            "WorkBuddy 安装包数字签名无效".to_string()
        } else {
            format!("WorkBuddy 安装包数字签名无效: {detail}")
        });
    }
    let subject = decode_command_output(&output.stdout);
    if !windows_signer_subject_is_trusted(&subject) {
        return Err("WorkBuddy 安装包签名方不是受信任的腾讯发行方".to_string());
    }
    Ok(())
}

async fn download_workbuddy_package<F>(
    client: &reqwest::Client,
    url: &str,
    expected_sha256: &str,
    destination: &Path,
    ...
    F: FnMut(DownloadProgress),
{
    ...
    #[cfg(target_os = "windows")]
    {
        if let Err(error) = verify_windows_authenticode(destination) {
            let _ = tokio::fs::remove_file(destination).await;
            return Err(error);
        }
        return Ok(());
    ...
    {
        let _ = tokio::fs::remove_file(destination).await;
        Err("WorkBuddy 官方源未提供 SHA-256，已拒绝无校验安装包".to_string())
    }
}

fn desktop_agent_candidates(agent_id: &str) -> Vec<PathBuf> {
    let mut paths = Vec::new();
    #[cfg(target_os = "windows")]
    {
        let mut add_root = |root: Option<String>, names: &[&str]| {
    ...
    #[cfg(target_os = "macos")]
    {
        ...
        let names: &[&str] = match agent_id {
            "codex-ui" => &["ChatGPT.app", "Codex.app"],
...
#[cfg(target_os = "windows")]
fn windows_app_package_installed(agent_id: &str) -> bool {
    let script = match agent_id {
        "codex-ui" => "$packages = Get-AppxPackage | Where-Object { $_.Name -like 'OpenAI.Codex*' -or $_.Name -like 'OpenAI.ChatGPT*' }; if ($packages) { exit 0 } else { exit 1 }",
        _ => return false,
    };
    let mut command = std::process::Command::new("powershell");
    command
...
#[cfg(not(target_os = "windows"))]
fn windows_app_package_installed(_agent_id: &str) -> bool {
    false
}
...
fn desktop_install_command(agent_id: &str) -> Result<String, String> {
    match agent_id {
        "codex-ui" => {
            #[cfg(target_os = "windows")]
            return Ok(
            ...
            #[cfg(target_os = "macos")]
            return Ok("官方 ChatGPT/Codex 安装器（自动下载并校验）".to_string());
            #[cfg(not(any(target_os = "windows", target_os = "macos")))]
            Err("Codex UI 仅支持 Windows 和 macOS".to_string())
        }
        "workbuddy" => workbuddy_platform_identifier()
            .map(|_| "官方 WorkBuddy 安装器（自动下载并校验）".to_string())
            .ok_or_else(|| "WorkBuddy 仅支持 Windows 和 macOS".to_string()),
        _ => Err(format!("Unsupported desktop agent: {agent_id}")),
    }
}

async fn install_workbuddy(app: &AppHandle) -> Result<(), String> {
    let platform = workbuddy_platform_identifier()
        .ok_or_else(|| "WorkBuddy 仅支持 Windows 和 macOS".to_string())?;
    emit_agent_install_output(
        app,
        "workbuddy",
        "stdout",
        "正在获取 WorkBuddy 官方安装包信息".to_string(),
    );
    ...
    for endpoint in workbuddy_update_endpoints() {
        let metadata_url = format!("{endpoint}?platform={platform}");
        emit_agent_install_progress(
            app,
            "workbuddy",
            "prepare",
            "running",
            Some(5),
            format!("正在尝试官方源 {endpoint}"),
        );
        let metadata = match client
            .get(&metadata_url)
            .send()
            .await
            .map_err(|error| format!("获取 WorkBuddy 安装包信息失败: {error}"))
            .and_then(|response| {
                response
                    .error_for_status()
                    .map_err(|error| format!("WorkBuddy 官方接口返回错误: {error}"))
            }) {
            Ok(response) => match response.json::<WorkBuddyUpdate>().await {
                Ok(metadata) => metadata,
                Err(error) => {
                    last_error = Some(format!("解析 WorkBuddy 安装包信息失败: {error}"));
                    continue;
                }
            },
            Err(error) => {
        ...
        let result = download_workbuddy_package(
            &client,
            &metadata.url,
    ...
    if !downloaded {
        return Err(last_error.unwrap_or_else(|| "WorkBuddy 官方源均不可用".to_string()));
    }
    emit_agent_install_progress(
        app,
        "workbuddy",
    ...
    let install_result = install_workbuddy_archive(archive_path.clone()).await;
    let _ = tokio::fs::remove_file(&archive_path).await;
    install_result?;

    if let Err(error) = launch_desktop_agent("workbuddy") {
        emit_agent_install_output(app, "workbuddy", "stderr", error);
    }
    emit_agent_install_progress(
        app,
        "workbuddy",
...
#[cfg(target_os = "macos")]
async fn install_codex_ui(app: &AppHandle) -> Result<(), String> {
    emit_agent_install_output(
        app,
        "codex-ui",
        "stdout",
        "正在获取官方 ChatGPT/Codex 桌面包信息".to_string(),
    );
    let client = reqwest::Client::new();
    let cask = client
        .get("https://formulae.brew.sh/api/cask/chatgpt.json")
        .send()
        .await
        .map_err(|error| format!("获取 ChatGPT/Codex 安装包信息失败: {error}"))?
        .error_for_status()
        .map_err(|error| format!("ChatGPT/Codex 官方接口返回错误: {error}"))?
        .json::<ChatGptCask>()
        .await
        .map_err(|error| format!("解析 ChatGPT/Codex 安装包信息失败: {error}"))?;
    let (url, sha256) = select_chatgpt_download(&cask)
    ...
    let install_result = install_macos_zip_app(archive_path.clone(), "ChatGPT.app").await;
    let _ = tokio::fs::remove_file(&archive_path).await;
    install_result?;
    if let Err(error) = launch_desktop_agent("codex-ui") {
        emit_agent_install_output(app, "codex-ui", "stderr", error);
    }
    emit_agent_install_progress(
        app,
        "codex-ui",
...
async fn install_workbuddy_archive(archive_path: PathBuf) -> Result<(), String> {
    #[cfg(target_os = "windows")]
    {
        tokio::task::spawn_blocking(move || {
            std::process::Command::new(&archive_path)
                .status()
                .map_err(|error| format!("启动 WorkBuddy 安装器失败: {error}"))
                .and_then(|status| {
                    if status.success() {
                        Ok(())
                    } else {
                        Err(format!("WorkBuddy 安装器退出，状态码: {:?}", status.code()))
                    }
                })
        })
        .await
        .map_err(|error| format!("WorkBuddy 安装任务失败: {error}"))?
    }
    ...
    {
        let _ = archive_path;
        Err("当前平台不支持安装 WorkBuddy".to_string())
    }
}
...
async fn install_macos_zip_app(archive_path: PathBuf, app_bundle: &str) -> Result<(), String> {
    ...
    tokio::task::spawn_blocking(move || {
        let extract_dir = std::env::temp_dir().join(format!(
            ...
            app_bundle.trim_end_matches(".app"),
            std::process::id()
        ));
        let _ = std::fs::remove_dir_all(&extract_dir);
        std::fs::create_dir_all(&extract_dir)
            .map_err(|error| format!("创建桌面应用解压目录失败: {error}"))?;
        let unzip_status = std::process::Command::new("ditto")
            .args(["-x", "-k"])
            .arg(&archive_path)
            .arg(&extract_dir)
            .status()
            .map_err(|error| format!("解压桌面应用安装包失败: {error}"))?;
...
#[cfg(target_os = "macos")]
fn find_app_bundle(root: &Path, bundle_name: &str) -> Option<PathBuf> {
    let direct = root.join(bundle_name);
    if direct.is_dir() {
        return Some(direct);
    }
    std::fs::read_dir(root)
        ...
        .find_map(|path| {
            if path.file_name().and_then(|name| name.to_str()) == Some(bundle_name) && path.is_dir()
            {
                Some(path)
            } else if path.is_dir() {
                find_app_bundle(&path, bundle_name)
            } else {
                None
            }
        })
...
fn launch_desktop_agent(agent_id: &str) -> Result<(), String> {
    ...
    #[cfg(target_os = "windows")]
    {
        if agent_id == "workbuddy" {
            if let Some(path) = desktop_agent_candidates(agent_id)
                .into_iter()
        ...
        if agent_id == "codex-ui" {
            let script = "$app = Get-StartApps | Where-Object { $_.Name -match 'Codex|ChatGPT' } | Select-Object -First 1; if (-not $app) { exit 1 }; Start-Process ('shell:AppsFolder\\' + $app.AppID)";
            let mut command = std::process::Command::new("powershell");
            command
                .args(["-NoProfile", "-NonInteractive", "-Command", script])
                .creation_flags(CREATE_NO_WINDOW);
        ...
        Err(format!("未找到 {agent_id} 的启动入口"))
    }

    #[cfg(not(any(target_os = "windows", target_os = "macos")))]
    {
        let _ = agent_id;
        Err("当前平台不支持启动桌面 Agent".to_string())
    }
}

#[cfg(target_os = "windows")]
fn node_portable_fallback_windows_command() -> String {
    let script = r#"$ErrorActionPreference='Stop'; $base='https://repo.huaweicloud.com/nodejs/latest-v22.x'; $content=(Invoke-WebRequest -UseBasicParsing ($base + '/SHASUMS256.txt')).Content; if ($content -is [byte[]]) { $content=[Text.Encoding]::UTF8.GetString($content) }; $arch=if ($env:PROCESSOR_ARCHITECTURE -eq 'ARM64') {'arm64'} else {'x64'}; $pattern='(?m)^([a-f0-9]{64})  (node-v[0-9.]+-win-' + $arch + '\.zip)$'; $match=[regex]::Match($content,$pattern); if (-not $match.Success) { throw 'Node.js archive metadata not found' }; $expected=$match.Groups[1].Value; $file=$match.Groups[2].Value; $archive=Join-Path $env:TEMP $file; $extract=Join-Path $env:TEMP ('cc-launch-node-' + $PID); Invoke-WebRequest -UseBasicParsing ($base + '/' + $file) -OutFile $archive; $actual=(Get-FileHash $archive -Algorithm SHA256).Hash.ToLowerInvariant(); if ($actual -ne $expected) { throw 'Node.js archive checksum mismatch' }; Remove-Item -LiteralPath $extract -Recurse -Force -ErrorAction SilentlyContinue; Expand-Archive -LiteralPath $archive -DestinationPath $extract -Force; $root=Get-ChildItem -LiteralPath $extract -Directory | Select-Object -First 1; $dest=Join-Path $env:LOCALAPPDATA 'cc-launch\node'; Remove-Item -LiteralPath $dest -Recurse -Force -ErrorAction SilentlyContinue; New-Item -ItemType Directory -Path $dest -Force | Out-Null; Copy-Item -Path (Join-Path $root.FullName '*') -Destination $dest -Recurse -Force; Remove-Item -LiteralPath $archive -Force -ErrorAction SilentlyContinue; Remove-Item -LiteralPath $extract -Recurse -Force -ErrorAction SilentlyContinue"#;
    format!(
...
#[cfg(target_os = "macos")]
fn node_portable_fallback_macos_command() -> String {
    r##"bash -c 'set -e; base=https://repo.huaweicloud.com/nodejs/latest-v22.x; arch=$(uname -m); if [ "$arch" = arm64 ]; then arch=arm64; else arch=x64; fi; sums=$(curl -fsSL "$base/SHASUMS256.txt"); entry=$(printf "%s\n" "$sums" | grep -E " node-v[0-9.]+-darwin-${arch}\.tar\.gz$" | head -n 1); [ -n "$entry" ]; hash=${entry%% *}; file=${entry##*  }; tmp=$(mktemp -d); curl -fsSL "$base/$file" -o "$tmp/$file"; actual=$(shasum -a 256 "$tmp/$file" | awk "{print \$1}"); [ "$actual" = "$hash" ]; dest="$HOME/.cc-launch/node"; rm -rf "$dest"; mkdir -p "$dest"; tar -xzf "$tmp/$file" -C "$dest" --strip-components=1; rm -rf "$tmp"'"##.to_string()
}

#[cfg(target_os = "windows")]
fn node_runtime_install_command() -> Option<String> {
    ...
    if command_exists("choco") {
        commands.push("choco install nodejs-lts -y".to_string());
    }
    if command_exists("scoop") {
        commands.push("call scoop install nodejs-lts".to_string());
    }
    commands.push(node_portable_fallback_windows_command());
    Some(commands.join(" || "))
}

#[cfg(target_os = "macos")]
fn node_runtime_install_command() -> Option<String> {
    let fallback = node_portable_fallback_macos_command();
    if command_exists("brew") {
        return Some(format!("brew install node || {fallback}"));
    }
    Some(fallback)
}
...
#[tauri::command]
pub async fn ensure_node_runtime(app: AppHandle) -> Result<NodeRuntimeStatus, String> {
    if let Some(version) = node_runtime_meets_requirement() {
    ...
    emit_agent_install_progress(
        ...
        "running",
        Some(0),
        "正在检查 Node.js 安装渠道；包管理器失败时将切换国内校验镜像",
    );

    let Some(command) = node_runtime_install_command() else {
        let error = if cfg!(target_os = "windows") {
            "未找到 winget、Chocolatey 或 Scoop，请从 https://nodejs.org 安装 Node.js LTS"
        } else if cfg!(target_os = "macos") {
            "未找到 Homebrew，请从 https://nodejs.org 安装 Node.js LTS，或先安装 Homebrew"
        } else {
            "当前平台没有可用的 Node.js 自动安装方式"
        };
        emit_agent_install_progress(&app, "node-runtime", "prepare", "error", None, error);
        return Ok(NodeRuntimeStatus {
            available: false,
    ...
    let result = tokio::task::spawn_blocking(move || {
        run_agent_install_with_output(&app, "node-runtime", &command_for_task)
    })
    .await
    .map_err(|error| format!("node runtime task join error: {error}"))?;

    match result {
        Ok(()) => {
            let version = node_runtime_meets_requirement();
        ...
        Err(error) => Ok(NodeRuntimeStatus {
            available: false,
            version: None,
            installed: true,
            command: Some(command),
...
fn collect_agent_preflight_context() -> AgentPreflightContext {
    let (npm_prefix_writable, _) = npm_global_prefix_status();
    AgentPreflightContext {
        platform: current_platform_name(),
        git_bash_available: windows_git_bash_available(),
        wsl_available: windows_wsl_distribution_available(),
        node_available: node_runtime_meets_requirement().is_some(),
        npm_prefix_writable,
        dotnet_available: windows_dotnet_runtime_available(),
        webview2_available: windows_webview2_available(),
    }
}

fn agent_dependencies(tool: &str) -> Vec<AgentInstallDependency> {
    let context = collect_agent_preflight_context();
    agent_dependencies_with_context(tool, context)
}

fn agent_dependencies_with_context(
    tool: &str,
    context: AgentPreflightContext,
) -> Vec<AgentInstallDependency> {
    let _ = context.webview2_available;
    dependencies_for_agent(
        tool,
        context.platform,
        context.git_bash_available,
        context.wsl_available,
        context.node_available,
        context.npm_prefix_writable,
        context.dotnet_available,
    )
}
...
fn dependency(
    ...
    AgentInstallDependency {
        ...
        action_url: action.and_then(dependency_action_url).map(str::to_string),
    }
}

fn dependency_action_url(name: &str) -> Option<&'static str> {
    match name {
        "Node.js" => Some("https://nodejs.org/en/download"),
        "Git Bash 或 WSL" => Some("https://git-scm.com/downloads"),
        "WSL" => Some("https://learn.microsoft.com/windows/wsl/install"),
        ".NET Runtime" => Some("https://dotnet.microsoft.com/download/dotnet"),
        _ => None,
    }
}

fn dependencies_for_agent(
    tool: &str,
    platform: &str,
    git_bash_available: bool,
    wsl_available: bool,
        ...
        return if platform == "windows" {
            vec![dependency(
                ...
                if dotnet_available {
                    ...
                    "WorkBuddy 启动异常时请检查 .NET Runtime"
                },
                "advisory",
                false,
                false,
                Some("安装 .NET Desktop Runtime"),
            )]
        } else {
            Vec::new()
        };
    ...
    let mut dependencies = vec![dependency(
        ...
        node_available,
        if node_available {
            "已检测到 Node.js 18+ 与 npm"
        } else {
            "未检测到 Node.js 18+ 与 npm"
        },
        "runtime",
        true,
        true,
    ...
    if !npm_prefix_writable {
        dependencies.push(dependency(
            "npm 全局目录权限",
            false,
            "当前 npm 全局目录不可写，将尝试切换到用户目录",
            "permission",
            true,
            true,
            Some("切换到用户目录"),
    ...
    if tool == "claude" && platform == "windows" {
        let shell_available = git_bash_available || wsl_available;
        dependencies.push(dependency(
            "Git Bash 或 WSL",
            shell_available,
            if shell_available {
                "已检测到 Claude Code 可用的 Bash 环境"
            } else {
                "Windows 需要 Git for Windows 的 Git Bash 或 WSL"
            },
            "shell",
            true,
            false,
            Some("安装 Git for Windows 或启用 WSL"),
        ));
    }

    if tool == "opencode" && platform == "windows" {
        dependencies.push(dependency(
            "WSL",
            wsl_available,
            if wsl_available {
                "已检测到 WSL，适合运行 OpenCode"
            } else {
                "OpenCode 在 Windows 推荐使用 WSL，当前 npm 安装仍可继续"
            },
            "advisory",
            false,
            false,
...
fn agent_install_command(tool: &str) -> Result<String, String> {
    let command = build_tool_lifecycle_command(&[tool], ToolLifecycleAction::Install, None)?;
    let command = if !npm_global_prefix_status().0 {
        ...
        #[cfg(not(target_os = "windows"))]
        {
            let shell = npm_install_shell();
            match (
                npm_install_command_for(tool),
                npm_install_command_with_user_prefix(tool, &npm_user_prefix_path()),
            ) {
                (Some(install), Some(user_install)) => command.replace(
                    &npm_command_chain_with_registry_fallback(install.to_string(), shell),
                    &user_install,
                ),
                _ => command,
            }
    ...
    if command.is_empty() {
        Err(format!("Unsupported tool action target: {tool}"))
    } else {
        Ok(command)
    }
}
...
pub async fn get_agent_install_statuses() -> Result<Vec<AgentInstallStatus>, String> {
    ...
    for spec in agent_install_specs() {
        if spec.desktop {
            let status = desktop_agent_status(spec.id);
            statuses.push(AgentInstallStatus {
                ...
                command: (!status.installed)
                    .then(|| desktop_install_command(spec.id))
                    .transpose()?,
                dependencies: agent_dependencies_with_context(spec.id, preflight),
            });
            continue;
        }
        ...
        statuses.push(AgentInstallStatus {
            ...
            unsupported_reason: spec.unsupported_reason.map(str::to_string),
            command: (!installed)
                .then(|| agent_install_command(tool))
                .transpose()?,
            dependencies: agent_dependencies_with_context(tool, preflight),
        });
    }
    Ok(statuses)
}
...
#[tauri::command]
pub async fn run_agent_install(
    app: AppHandle,
    agent_id: String,
    ...
    if spec.desktop {
        if !spec.supported {
            return Err(spec
                .unsupported_reason
                .unwrap_or("此 Agent 暂不支持自动安装")
                .to_string());
        }
        if agent_id == "workbuddy" {
            let result = install_workbuddy(&app).await;
            ...
            result?;
        } else if agent_id == "codex-ui" {
            #[cfg(target_os = "macos")]
            {
                let result = install_codex_ui(&app).await;
                if let Err(error) = &result {
                    emit_agent_install_progress(
                        &app,
                        "codex-ui",
            ...
            {
                let command = desktop_install_command(&agent_id)?;
                let event_agent_id = agent_id.clone();
                tokio::task::spawn_blocking(move || {
                    run_agent_install_with_output(&app, &event_agent_id, &command)
                })
                .await
                .map_err(|error| format!("desktop agent install task join error: {error}"))??;
                if let Err(error) = launch_desktop_agent(&agent_id) {
                    log::warn!("failed to launch desktop agent {agent_id}: {error}");
                }
            }
        } else {
            let command = desktop_install_command(&agent_id)?;
        ...
        return Ok(AgentInstallResult { success: true });
    }
    let tool = spec.tool.filter(|_| spec.supported).ok_or_else(|| {
        spec.unsupported_reason
            .unwrap_or("此 Agent 暂不支持自动安装")
            .to_string()
    })?;
    if let Some(dependency) = blocking_dependency(&agent_dependencies(tool)) {
        let error = format!("缺少必要环境：{}。{}", dependency.name, dependency.detail);
        ...
        && !node_runtime_meets_requirement().is_some()
    {
        ...
        if !runtime.available {
            return Err(runtime
                .error
                .unwrap_or_else(|| "Node.js 18+ 与 npm 不可用，无法安装此 Agent".to_string()));
        }
    }
    let command = agent_install_command(tool)?;
    emit_agent_install_progress(
        &app,
        &agent_id,
        "prepare",
...
fn emit_agent_install_progress(
    app: &AppHandle,
    agent_id: &str,
    phase: &str,
    status: &str,
...
fn run_agent_install_with_output(
    ...
    match &result {
        ...
        Err(error) => emit_agent_install_progress(
            app,
            agent_id,
            "install",
            "error",
...
fn run_agent_install_with_output_inner(
    app: &AppHandle,
    agent_id: &str,
    command_line: &str,
) -> Result<(), String> {
    ...
    let bat_file = std::env::temp_dir().join(format!(
        "cc_switch_agent_install_{}_{}.bat",
        agent_id,
        std::process::id()
    ));
    #[cfg(target_os = "windows")]
    std::fs::write(&bat_file, command_line)
        .map_err(|error| format!("写入批处理文件失败: {error}"))?;

    #[cfg(target_os = "windows")]
    let mut process = {
        let mut value = Command::new("cmd");
        value.arg("/C").arg(&bat_file);
    ...
    let mut child = process
        .env("PATH", merged_node_path())
        .stdout(Stdio::piped())
        .stderr(Stdio::piped())
        .spawn()
        .map_err(|error| format!("启动安装进程失败: {error}"))?;
    let stdout = child
        .stdout
        .take()
        .ok_or_else(|| "无法读取安装标准输出".to_string())?;
    let stderr = child
        .stderr
        .take()
        .ok_or_else(|| "无法读取安装错误输出".to_string())?;
    ...
    let status = child
        .wait()
        .map_err(|error| format!("等待安装进程失败: {error}"))?;
    #[cfg(target_os = "windows")]
...
fn merged_node_path() -> String {
    ...
    let prefix = std::env::join_paths(node_dirs)
    ...
    if current.is_empty() {
        ...
        format!("{prefix}{separator}{current}")
    }
}
...
const NPM_REGISTRY_FALLBACKS: [&str; 2] = [
    "https://registry.npmmirror.com/",
    "https://repo.huaweicloud.com/repository/npm/",
];

const WORKBUDDY_UPDATE_ENDPOINTS: [&str; 2] = [
    "https://www.workbuddy.ai/v2/update",
    "https://www.codebuddy.cn/v2/update",
];

fn workbuddy_update_endpoints() -> &'static [&'static str] {
    &WORKBUDDY_UPDATE_ENDPOINTS
}

fn npm_install_command_with_registry_fallback(
    tool: &str,
    shell: LifecycleCommandShell,
) -> Option<String> {
    let install = npm_install_command_for(tool)?;
    Some(npm_command_chain_with_registry_fallback(
        install.to_string(),
        shell,
    ))
}

fn npm_command_chain_with_registry_fallback(
    install: String,
    shell: LifecycleCommandShell,
) -> String {
    let mut command = install.clone();
    for registry in NPM_REGISTRY_FALLBACKS {
        command = chain_update_commands(command, format!("{install} --registry={registry}"), shell);
    }
    command
}

fn npm_user_prefix_path() -> String {
    #[cfg(target_os = "windows")]
    if let Ok(local_app_data) = std::env::var("LOCALAPPDATA") {
    ...
    ".cc-launch/npm".to_string()
}

fn npm_user_bin_dir() -> PathBuf {
    let prefix = PathBuf::from(npm_user_prefix_path());
    if cfg!(target_os = "windows") {
        prefix
    } else {
        prefix.join("bin")
...
fn npm_install_command_with_user_prefix(tool: &str, prefix: &str) -> Option<String> {
    let install = npm_install_command_for(tool)?;
    let install = install.replacen("npm ", &format!("npm {} ", npm_prefix_argument(prefix)), 1);
    let shell = npm_install_shell();
    Some(npm_command_chain_with_registry_fallback(install, shell))
}
...
fn install_gate(available: bool, blocking: bool) -> InstallGate {
...
fn blocking_dependency(dependencies: &[AgentInstallDependency]) -> Option<&AgentInstallDependency> {
    dependencies.iter().find(|dependency| {
        matches!(
            install_gate(dependency.available, dependency.blocking),
            InstallGate::Blocked
...
#[cfg(target_os = "windows")]
fn npm_prefix_argument(prefix: &str) -> String {
    format!("--prefix {}", win_quote_path_for_batch(prefix))
}

#[cfg(not(target_os = "windows"))]
fn npm_prefix_argument(prefix: &str) -> String {
    format!("--prefix {}", shell_single_quote(prefix))
}

fn npm_install_shell() -> LifecycleCommandShell {
    #[cfg(target_os = "windows")]
    {
...
fn tool_action_shell_command_for_shell(
        ...
        && matches!(shell, LifecycleCommandShell::WindowsBatch)
    {
        return Some(chain_update_commands(
            ...
            npm_install_command_with_registry_fallback(tool, shell)?,
            shell,
        ));
    }
    ...
    let install = npm_install_command_with_registry_fallback(tool, shell)?;
    match action {
...
fn build_tool_search_paths(tool: &str) -> Vec<std::path::PathBuf> {
    ...
    push_unique_path(&mut search_paths, npm_user_bin_dir());
...
#[cfg(target_os = "windows")]
fn claude_npm_shim_preflight_error(tool_path: &Path) -> Option<&'static str> {
    const ERROR: &str = "Claude Code native binary is missing or invalid; reinstall @anthropic-ai/claude-code with npm optional dependencies enabled.";
    ...
    let shim = std::fs::read_to_string(tool_path).ok()?;
    let normalized_shim = shim.replace('/', "\\").to_ascii_lowercase();
    if !normalized_shim.contains("node_modules\\@anthropic-ai\\claude-code\\bin\\claude.exe") {
        return None;
    }

    let native_binary = tool_path
        .parent()?
        .join("node_modules")
        .join("@anthropic-ai")
        .join("claude-code")
        .join("bin")
        .join("claude.exe");
    match std::fs::read(&native_binary) {
        Ok(bytes) if bytes.starts_with(b"MZ") => None,
        _ => Some(ERROR),
    }
}
...
fn probe_path_default_version(tool: &str) -> ShellProbe {
    ...
    if let Some(error) = claude_npm_shim_preflight_error(&path_default) {
        return ShellProbe::FoundButFailed(error.to_string());
    }
...
fn scan_cli_version(tool: &str) -> ShellProbe {
    ...
    for path in &search_paths {
        ...
        for tool_path in tool_executable_candidates(tool, path) {
            ...
            #[cfg(target_os = "windows")]
            if let Some(error) = claude_npm_shim_preflight_error(&tool_path) {
                exec_diagnostic.get_or_insert_with(|| error.to_string());
                continue;
            }
...
fn installer_with_npm_fallback(installer: &str, tool: &str) -> String {
    match npm_install_command_with_registry_fallback(tool, LifecycleCommandShell::Posix) {
        Some(npm) => {
...
mod tests {
        ...
        for action in [ToolLifecycleAction::Install, ToolLifecycleAction::Update] {
            let command =
                tool_action_shell_command_for_shell("grok", action, LifecycleCommandShell::Posix)
                    .expect("Grok should have a package-manager command");
            assert!(command.starts_with("npm i -g @xai-official/grok@latest"));
            assert!(command.contains("registry.npmmirror.com"));
            assert!(command.contains("repo.huaweicloud.com/repository/npm"));
        }

        // Static update remains package-manager based. Only a path positively
        // identified as xAI's native install may run `grok update`.
        let windows_command = tool_action_shell_command_for_shell(
            "grok",
            ToolLifecycleAction::Update,
            LifecycleCommandShell::WindowsBatch,
        )
        .expect("Grok should have a Windows package-manager command");
        assert!(windows_command.starts_with("npm i -g @xai-official/grok@latest"));
        assert!(windows_command.contains("registry.npmmirror.com"));
    }
    ...
    mod anchored_upgrade_windows {
        ...
        fn opencode_windows_static_fallback_skips_official_upgrade() {
            ...
            assert!(cmd.starts_with("npm i -g opencode-ai@latest"));
            assert!(cmd.contains("registry.npmmirror.com"));
        ...
        fn grok_windows_install_prefers_powershell_with_npm_fallback() {
            ...
            assert!(
                install.contains("|| call npm i -g @xai-official/grok@latest"),
                "npm fallback should remain available: {install}"
            );
            assert!(install.contains("registry.npmmirror.com"));
            assert!(install.contains("repo.huaweicloud.com/repository/npm"));
    ...
    mod windows_helpers {
        ...
        #[test]
        fn claude_npm_shim_preflight_rejects_text_placeholder_binary() {
            ...
            let binary = dir
                ...
                .join("bin")
                .join("claude.exe");
            std::fs::create_dir_all(binary.parent().expect("binary should have a parent"))
                .expect("native binary directory should be created");
            std::fs::write(
                &shim,
                "@ECHO off\r\nGOTO start\r\n:start\r\nSETLOCAL\r\nSET dp0=%~dp0\r\n\"%dp0%\\node_modules\\@anthropic-ai\\claude-code\\bin\\claude.exe\" %*\r\n",
            )
            .expect("shim should be created");
            std::fs::write(
                &binary,
        ...
        fn wsl_install_uses_posix_install_priority() {
            ...
            assert!(codex.starts_with("npm i -g @openai/codex@latest"));
            assert!(codex.contains("registry.npmmirror.com"));
            assert!(codex.contains("repo.huaweicloud.com/repository/npm"));
        ...
        fn wsl_npm_tools_use_posix_update_chain_without_batch_call() {
            ...
            assert!(cmd.starts_with("claude update || npm i -g @anthropic-ai/claude-code@latest"));
            assert!(cmd.contains("registry.npmmirror.com"));
            assert!(cmd.contains("repo.huaweicloud.com/repository/npm"));
        }
    }
    ...
    mod install_strategy {
        ...
        fn claude_install_prefers_native_with_npm_fallback() {
            ...
            assert!(parts.len() >= 2, "should be a short-circuit chain");
        ...
        fn codex_install_keeps_static_npm() {
            ...
            assert!(cmd.starts_with("npm i -g @openai/codex@latest"));
            assert!(cmd.contains("registry.npmmirror.com"));
            assert!(cmd.contains("repo.huaweicloud.com/repository/npm"));
        }
        ...
        fn gemini_install_keeps_static_npm() {
            ...
            assert!(cmd.starts_with("npm i -g @google/gemini-cli@latest"));
            assert!(cmd.contains("registry.npmmirror.com"));
        }
        ...
        fn openclaw_install_keeps_static_npm() {
            ...
            assert!(cmd.starts_with("npm i -g openclaw@latest"));
            assert!(cmd.contains("registry.npmmirror.com"));
        }
        ...
        fn pi_install_uses_the_verified_pinned_package() {
            ...
            assert!(cmd.starts_with("npm i -g @earendil-works/pi-coding-agent@latest"));
            assert!(cmd.contains("registry.npmmirror.com"));
        }
        ...
        fn update_fallbacks_use_official_cli_only_when_supported() {
            ...
            assert!(
                claude.starts_with("claude update || npm i -g @anthropic-ai/claude-code@latest")
            );
            assert!(claude.contains("registry.npmmirror.com"));
            ...
            assert!(gemini.starts_with("npm i -g @google/gemini-cli@latest"));
            assert!(gemini.contains("registry.npmmirror.com"));
            assert!(!static_fallback_command("gemini").contains("gemini update"));
            let grok = static_fallback_command("grok");
            assert!(grok.starts_with("npm i -g @xai-official/grok@latest"));
            assert!(grok.contains("registry.npmmirror.com"));
            assert!(!static_fallback_command("grok").contains("grok update"));
            let opencode = static_fallback_command("opencode");
            assert!(opencode.starts_with("opencode upgrade || npm i -g opencode-ai@latest"));
            assert!(opencode.contains("registry.npmmirror.com"));
            let openclaw = static_fallback_command("openclaw");
            assert!(openclaw.starts_with("openclaw update --yes || npm i -g openclaw@latest"));
            assert!(openclaw.contains("registry.npmmirror.com"));
            let pi = static_fallback_command("pi");
            assert!(pi.starts_with("npm i -g @earendil-works/pi-coding-agent@latest"));
            assert!(pi.contains("registry.npmmirror.com"));
    ...
    fn agent_install_specs_cover_every_managed_app_and_manual_desktop_item_is_explicit() {
        ...
        let desktop = specs
            ...
            .expect("Claude Desktop must be shown in the install panel");
        assert!(!desktop.supported);
        assert!(desktop.unsupported_reason.is_some());
        assert!(specs
            .iter()
            .filter(|spec| spec.supported)
            .all(|spec| spec.tool.is_some() || spec.desktop));
    }

    #[test]
    fn node_major_version_accepts_node_version_prefix_and_rejects_invalid_values() {
        assert_eq!(node_major_version("v22.18.0"), Some(22));
        assert_eq!(node_major_version("18.20.4"), Some(18));
        assert_eq!(node_major_version("node"), None);
    }

    #[test]
    fn parse_install_progress_reads_only_percent_tokens() {
        assert_eq!(parse_install_progress("downloading 42%"), Some(42));
    ...
    #[test]
    fn npm_install_fallback_includes_domestic_registries_after_official_registry() {
        let command =
            npm_install_command_with_registry_fallback("codex", LifecycleCommandShell::Posix)
                .expect("Codex should have an npm install command");

        let official_index = command
            .find("npm i -g @openai/codex@latest")
            .expect("official npm command should be first");
        let npmmirror_index = command
            .find("registry.npmmirror.com")
            .expect("npmmirror fallback should be present");
        let huawei_index = command
            .find("repo.huaweicloud.com/repository/npm")
            .expect("Huawei Cloud fallback should be present");

        assert!(official_index < npmmirror_index);
        assert!(npmmirror_index < huawei_index);
    }

    #[cfg(target_os = "windows")]
    #[test]
    fn node_runtime_fallback_ends_with_verified_portable_install() {
        let command = node_runtime_install_command().expect("Windows should have a fallback");

        assert!(command.contains("powershell -NoProfile"));
        assert!(command.contains("-EncodedCommand"));
    }

    #[test]
    fn desktop_agent_specs_include_codex_ui_and_workbuddy() {
        let specs = agent_install_specs();
        let codex_ui = specs
            .iter()
            .find(|spec| spec.id == "codex-ui")
            .expect("Codex UI spec should exist");
        let workbuddy = specs
            .iter()
            .find(|spec| spec.id == "workbuddy")
            .expect("WorkBuddy spec should exist");
    ...
    #[test]
    fn workbuddy_sha256_validation_is_case_insensitive() {
        let payload = b"workbuddy installer";
        let digest = sha256_hex(payload);

        assert!(sha256_matches(payload, &digest.to_uppercase()));
    ...
    fn claude_windows_dependency_blocks_without_git_bash_or_wsl() {
        let deps = dependencies_for_agent("claude", "windows", false, false, true, true, false);
        assert!(deps
            .iter()
            .any(|item| { item.name == "Git Bash 或 WSL" && item.blocking && !item.available }));
    }

    #[test]
    fn opencode_windows_wsl_is_advisory_only() {
        let deps = dependencies_for_agent("opencode", "windows", false, false, true, true, false);
        assert!(deps
            .iter()
            .any(|item| item.name == "WSL" && !item.blocking && !item.available));
    }

    #[test]
    fn windows_dotnet_and_webview2_are_non_blocking_advisories() {
        let workbuddy =
            dependencies_for_agent("workbuddy", "windows", true, false, false, false, false);
    ...
    fn dotnet_runtime_parser_accepts_desktop_runtime() {
        assert!(has_dotnet_runtime(
            "Microsoft.WindowsDesktop.App 8.0.1 [C:\\Program Files\\dotnet\\shared]"
        ));
        assert!(!has_dotnet_runtime(""));
    }
    ...
    fn npm_install_command_can_use_user_prefix_without_global_registry_change() {
        let command =
            npm_install_command_with_user_prefix("codex", r"C:\Users\test\.cc-launch\npm")
    ...
    fn blocked_dependency_prevents_agent_command_start() {
        assert_eq!(install_gate(false, true), InstallGate::Blocked);
        assert_eq!(install_gate(true, true), InstallGate::Allowed);
        assert_eq!(install_gate(false, false), InstallGate::Allowed);
    }

    #[test]
    fn dependency_actions_use_fixed_first_party_urls() {
        assert_eq!(
            dependency_action_url("Git Bash 或 WSL"),
            Some("https://git-scm.com/downloads")
        );
        assert_eq!(
            dependency_action_url("WSL"),
            Some("https://learn.microsoft.com/windows/wsl/install")
        );
        assert_eq!(dependency_action_url("unknown"), None);
    }

    #[test]
    fn unsupported_agent_does_not_get_blocked_by_unrelated_runtime_dependency() {
        let spec = agent_install_specs()
            .into_iter()
            .find(|spec| spec.id == "claude-desktop")
            .expect("Claude Desktop should be declared");
        assert!(!spec.supported);
        assert!(spec.tool.is_none());
    }

    #[test]
    fn workbuddy_expected_hash_prefers_sha256hash_and_accepts_hash_alias() {
        let primary = WorkBuddyUpdate {
            url: "https://example.com/workbuddy.exe".to_string(),
            sha256hash: "primary".to_string(),
            hash: "alias".to_string(),
        };
        assert_eq!(workbuddy_expected_hash(&primary), "primary");

        let alias = WorkBuddyUpdate {
            url: "https://example.com/workbuddy.exe".to_string(),
            sha256hash: String::new(),
            hash: "alias".to_string(),
        };
        assert_eq!(workbuddy_expected_hash(&alias), "alias");
    }

    #[cfg(target_os = "windows")]
    #[test]
    fn windows_workbuddy_signature_policy_requires_tencent_subject() {
        assert!(windows_signer_subject_is_trusted(
            "CN=Tencent Technology (Shenzhen) Company Limited"
        ));
        assert!(!windows_signer_subject_is_trusted("CN=Unknown Publisher"));
    }

    #[test]
    fn workbuddy_platform_identifier_matches_supported_targets() {
        let platform = workbuddy_platform_identifier();

        #[cfg(target_os = "windows")]
        assert_eq!(platform, Some("workbuddy-win32-x64-user"));
        #[cfg(all(target_os = "macos", target_arch = "aarch64"))]
        assert_eq!(platform, Some("workbuddy-darwin-arm64"));
        #[cfg(all(target_os = "macos", target_arch = "x86_64"))]
        assert_eq!(platform, Some("workbuddy-darwin-x64"));
        #[cfg(not(any(target_os = "windows", target_os = "macos")))]
        assert_eq!(platform, None);
    }

    #[test]
    fn workbuddy_fallbacks_stay_on_tencent_official_domains() {
        let endpoints = workbuddy_update_endpoints();

        assert_eq!(endpoints.len(), 2);
        assert!(endpoints[0].contains("workbuddy.ai"));
        assert!(endpoints[1].contains("codebuddy.cn"));
    }

    #[test]
    fn desktop_install_commands_use_first_party_channels() {
        let codex = desktop_install_command("codex-ui");
        let workbuddy = desktop_install_command("workbuddy");

        #[cfg(target_os = "windows")]
        assert!(codex.unwrap().contains("9PLM9XGG6VKS"));
        #[cfg(target_os = "macos")]
        assert!(codex.unwrap().contains("ChatGPT/Codex"));
        #[cfg(not(any(target_os = "windows", target_os = "macos")))]
        assert!(codex.is_err());

        #[cfg(any(target_os = "windows", target_os = "macos"))]
        assert!(workbuddy.unwrap().contains("WorkBuddy"));
        #[cfg(not(any(target_os = "windows", target_os = "macos")))]
        assert!(workbuddy.is_err());
    }

    #[test]
    fn chatgpt_cask_download_selects_the_x64_variant_on_x64_targets() {
        let cask = ChatGptCask {
            url: "https://example.test/ChatGPT-darwin-arm64.zip".to_string(),
            sha256: "arm-hash".to_string(),
            variations: HashMap::from([(
                "ventura".to_string(),
                ChatGptCaskVariation {
                    url: Some("https://example.test/ChatGPT-darwin-x64.zip".to_string()),
                    sha256: Some("x64-hash".to_string()),
                },
            )]),
        };
        let (url, sha256) = select_chatgpt_download(&cask).expect("cask should resolve");

        #[cfg(target_arch = "x86_64")]
        assert_eq!(
            (url, sha256),
            (
                "https://example.test/ChatGPT-darwin-x64.zip".to_string(),
                "x64-hash".to_string()
            )
        );
        ...
        assert_eq!(
            (url, sha256),
            (
                "https://example.test/ChatGPT-darwin-arm64.zip".to_string(),
                "arm-hash".to_string()
            )
        );
    }
