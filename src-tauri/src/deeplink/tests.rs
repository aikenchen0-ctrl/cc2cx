...
impl TestHomeGuard {
    fn new() -> Self {
        ...
        let original_test_home = env::var_os("CC_LAUNCH_TEST_HOME");

        env::set_var("HOME", dir.path());
        env::set_var("USERPROFILE", dir.path());
        env::set_var("CC_LAUNCH_TEST_HOME", dir.path());
...
impl Drop for TestHomeGuard {
    fn drop(&mut self) {
        match &self.original_test_home {
            Some(value) => env::set_var("CC_LAUNCH_TEST_HOME", value),
            None => env::remove_var("CC_LAUNCH_TEST_HOME"),
        }
...
fn test_parse_valid_claude_deeplink() {
    let url = "cclaunch://v1/import?resource=provider&app=claude&name=Test%20Provider&homepage=https%3A%2F%2Fexample.com&endpoint=https%3A%2F%2Fapi.example.com&apiKey=sk-test-123&icon=claude";
...
fn test_parse_deeplink_with_notes() {
    let url = "cclaunch://v1/import?resource=provider&app=codex&name=Codex&homepage=https%3A%2F%2Fcodex.com&endpoint=https%3A%2F%2Fapi.codex.com&apiKey=key123&notes=Test%20notes";
...
fn pi_provider_deeplink_is_not_a_second_add_provider_entry() {
    let url = "cclaunch://v1/import?resource=provider&app=pi&name=Pi";
...
fn test_parse_grokbuild_provider() {
    ...
    let url = "cclaunch://v1/import?resource=provider&app=grokbuild&name=Grok%20Relay&endpoint=https%3A%2F%2Fapi.example.com%2Fv1&apiKey=secret&model=grok-4.5";
...
fn test_parse_unsupported_version() {
    let url = "cclaunch://v2/import?resource=provider&app=claude&name=Test";
...
fn test_parse_missing_required_field() {
    ...
    let url = "cclaunch://v1/import?resource=provider&app=claude";
...
fn test_import_prompt_allows_space_in_base64_content() {
    ...
    let url = "cclaunch://v1/import?resource=prompt&app=codex&name=PromptPlus&content=Pj4+";
    let request = parse_deeplink_url(url).unwrap();
...
fn test_parse_prompt_deeplink() {
    ...
    let url = format!(
        "cclaunch://v1/import?resource=prompt&app=claude&name=test&content={}&description=desc&enabled=true",
        content_b64
    );
...
fn test_parse_grokbuild_prompt_deeplink() {
    ...
    let url = format!(
        "cclaunch://v1/import?resource=prompt&app=grokbuild&name=test&content={content_b64}"
    );
...
fn test_parse_mcp_deeplink() {
    ...
    let url = format!(
        "cclaunch://v1/import?resource=mcp&apps=claude,codex&config={}&enabled=true",
        config_b64
    );
...
fn test_parse_grokbuild_mcp_deeplink() {
    ...
    let config_b64 = BASE64_STANDARD.encode(config);
    let url = format!(
        "cclaunch://v1/import?resource=mcp&apps=grokbuild&config={config_b64}&enabled=true"
    );
...
fn test_parse_skill_deeplink() {
    let url = "cclaunch://v1/import?resource=skill&repo=owner/repo&directory=skills&branch=dev";
    let request = parse_deeplink_url(url).unwrap();
...
fn test_parse_multiple_endpoints_comma_separated() {
    let url = "cclaunch://v1/import?resource=provider&app=claude&name=Test&endpoint=https%3A%2F%2Fapi1.example.com,https%3A%2F%2Fapi2.example.com,https%3A%2F%2Fapi3.example.com&apiKey=sk-test";
...
fn test_parse_single_endpoint_backward_compatible() {
    ...
    let url = "cclaunch://v1/import?resource=provider&app=claude&name=Test&endpoint=https%3A%2F%2Fapi.example.com&apiKey=sk-test";
...
fn test_parse_endpoints_with_spaces_trimmed() {
    let url = "cclaunch://v1/import?resource=provider&app=claude&name=Test&endpoint=https%3A%2F%2Fapi1.example.com%20,%20https%3A%2F%2Fapi2.example.com&apiKey=sk-test";
