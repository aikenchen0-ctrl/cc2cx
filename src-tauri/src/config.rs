...
pub const APP_CONFIG_DIR_NAME: &str = ".cc-launch";
pub const DATABASE_FILE_NAME: &str = "cc-launch.db";
...
///   且不一定等于用户目录，可能导致 `.cc-launch/cc-launch.db` 路径变化，从而“看起来像数据丢失”。
///
/// ## 测试隔离
///
/// 为了让 Windows CI/本地测试能稳定隔离真实用户数据，可通过 `CC_LAUNCH_TEST_HOME`
...
mod tests {
    ...
    #[test]
    fn app_config_dir_uses_cc_launch_directory_under_test_home() {
        let test_home = tempfile::tempdir().unwrap();
        std::env::set_var(TEST_HOME_ENV, test_home.path());

        assert_eq!(
            get_app_config_dir(),
            test_home.path().join(APP_CONFIG_DIR_NAME),
        );
    ...
    #[test]
    fn database_init_uses_cc_launch_database_without_creating_legacy_database() {
        let test_home = tempfile::tempdir().unwrap();
        std::env::set_var(TEST_HOME_ENV, test_home.path());

        crate::database::Database::init().unwrap();
        ...
        assert!(test_home
            ...
            .exists());
        assert!(
            !test_home.path().join(".cc-switch/cc-switch.db").exists(),
            "a new cc-launch installation must not create legacy cc-switch data",
        );
