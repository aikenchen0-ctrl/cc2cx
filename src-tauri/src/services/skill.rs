...
impl SkillService {
    ...
    pub fn get_app_skills_dir(app: &AppType) -> Result<PathBuf> {
        ...
        // 默认路径：回退到用户主目录下的标准位置。
        // 必须走 get_home_dir()（可被 CC_LAUNCH_TEST_HOME 覆盖）：Windows 上 dirs::home_dir()
...
mod tests {
    ...
    /// CC_LAUNCH_TEST_HOME 隔离守卫（serial 测试间互斥由 #[serial] 保证，
    /// 守卫只负责在测试结束后恢复原值）。
    struct TestHomeGuard(Option<std::ffi::OsString>);
    impl TestHomeGuard {
        fn set(home: &Path) -> Self {
            let guard = Self(std::env::var_os("CC_LAUNCH_TEST_HOME"));
            std::env::set_var("CC_LAUNCH_TEST_HOME", home);
            guard
        }
    }
    impl Drop for TestHomeGuard {
        fn drop(&mut self) {
            match self.0.take() {
                Some(value) => std::env::set_var("CC_LAUNCH_TEST_HOME", value),
                None => std::env::remove_var("CC_LAUNCH_TEST_HOME"),
            }
        }
    }
    ...
    #[test]
    // serial：与 backup/s3_sync/deeplink 等同样读写进程级 CC_LAUNCH_TEST_HOME 的测试互斥，
    // EnvGuard 只负责恢复不提供互斥。
    #[serial_test::serial]
    fn get_app_skills_dir_honors_test_home_override() {
        // 回归：曾直呼 dirs::home_dir() 绕过 CC_LAUNCH_TEST_HOME——Unix 上碰巧跟 $HOME
        ...
        let temp = tempdir().expect("tempdir");
        let _guard = EnvGuard(std::env::var_os("CC_LAUNCH_TEST_HOME"));
        std::env::set_var("CC_LAUNCH_TEST_HOME", temp.path());
