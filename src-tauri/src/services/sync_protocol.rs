...
mod tests {
    ...
    #[test]
    fn validate_manifest_compat_rejects_legacy_cc_switch_format() {
        let manifest = manifest_with(
            "cc-switch-webdav-sync",
            PROTOCOL_VERSION,
            Some(DB_COMPAT_VERSION),
        );
        let error = validate_manifest_compat(&manifest, RemoteLayout::Current)
            .expect_err("legacy cc-switch manifests must not be accepted");
