...
/// Gemini OAuth Client ID 通过运行环境提供，避免将客户端凭据写入仓库历史。
const GEMINI_OAUTH_CLIENT_ID_ENV: &str = "CC_LAUNCH_GEMINI_OAUTH_CLIENT_ID";
const GEMINI_OAUTH_CLIENT_SECRET_ENV: &str = "CC_LAUNCH_GEMINI_OAUTH_CLIENT_SECRET";

fn gemini_oauth_client_id_from(value: Option<String>) -> Option<String> {
    value.filter(|client_id| !client_id.trim().is_empty())
}

fn gemini_oauth_client_id() -> Result<String, String> {
    gemini_oauth_client_id_from(std::env::var(GEMINI_OAUTH_CLIENT_ID_ENV).ok()).ok_or_else(|| {
        format!(
            "Gemini token refresh requires the {GEMINI_OAUTH_CLIENT_ID_ENV} environment variable."
        )
    })
}

fn gemini_oauth_client_secret_from(value: Option<String>) -> Option<String> {
    value.filter(|secret| !secret.trim().is_empty())
}

fn gemini_oauth_client_secret() -> Result<String, String> {
    gemini_oauth_client_secret_from(std::env::var(GEMINI_OAUTH_CLIENT_SECRET_ENV).ok())
        .ok_or_else(|| {
            format!(
                "Gemini token refresh requires the {GEMINI_OAUTH_CLIENT_SECRET_ENV} environment variable."
            )
        })
}
...
async fn refresh_gemini_token(refresh_token: &str) -> Result<Option<String>, String> {
    ...
    let resp = client
        ...
        .send()
        .await
        .map_err(|error| format!("Gemini token refresh request failed: {error}"))?;
    ...
    let body: serde_json::Value = resp
        .json()
        .await
        .map_err(|error| format!("Failed to parse Gemini token refresh response: {error}"))?;
    Ok(body
        .get("access_token")
...
mod tests {
    ...
    #[test]
    fn gemini_oauth_client_secret_requires_a_non_empty_value() {
        assert_eq!(gemini_oauth_client_secret_from(None), None);
        assert_eq!(
            gemini_oauth_client_secret_from(Some("   ".to_string())),
            None
        );
        assert_eq!(
            gemini_oauth_client_secret_from(Some("secret".to_string()),),
            Some("secret".to_string())
        );
    }

    #[test]
    fn gemini_oauth_client_id_requires_a_non_empty_value() {
        assert_eq!(gemini_oauth_client_id_from(None), None);
        assert_eq!(gemini_oauth_client_id_from(Some("   ".to_string())), None);
        assert_eq!(
            gemini_oauth_client_id_from(Some("client-id".to_string())),
