...
use crate::config::{get_app_config_dir, DATABASE_FILE_NAME};
...
impl Database {
    ...
    {
        let db_path = get_app_config_dir().join(DATABASE_FILE_NAME);
        if !db_path.exists() {
            return Ok(None);
        }
...
mod tests {
    ...
    impl TestHomeGuard {
        fn new() -> Self {
            ...
            let previous_test_home = std::env::var_os("CC_LAUNCH_TEST_HOME");
            std::env::set_var("CC_LAUNCH_TEST_HOME", temp_dir.path());
            ...
            // CC_LAUNCH_TEST_HOME and makes import exercise its safety backup.
            let config_dir = temp_dir.path().join(".cc-launch");
            std::fs::create_dir_all(&config_dir).expect("create isolated config directory");
            std::fs::File::create(config_dir.join("cc-launch.db"))
    ...
    impl Drop for TestHomeGuard {
        fn drop(&mut self) {
            match self.previous_test_home.as_ref() {
                Some(previous) => std::env::set_var("CC_LAUNCH_TEST_HOME", previous),
