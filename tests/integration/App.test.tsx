...
const agentInstallMocks = vi.hoisted(() => ({
  getStatuses: vi.fn(),
}));
...
vi.mock("@/lib/api/agentInstall", () => ({
  agentInstallApi: {
    getStatuses: agentInstallMocks.getStatuses,
  },
}));

vi.mock("@/components/agent-install/AgentInstallPanel", () => ({
  AgentInstallPanel: ({ isOpen }: { isOpen: boolean }) =>
    isOpen ? <div data-testid="agent-install-panel" /> : null,
}));
...
describe("App integration with MSW", () => {
  beforeEach(() => {
    ...
    localStorage.removeItem("cc-launch-agent-first-scan-complete");
    agentInstallMocks.getStatuses.mockResolvedValue([]);
  });

  it("opens the agent installer after the first scan finds a missing agent", async () => {
    agentInstallMocks.getStatuses.mockResolvedValueOnce([
      {
        id: "codex",
        name: "Codex",
    ...
    await waitFor(() =>
      expect(screen.getByTestId("agent-install-panel")).toBeInTheDocument(),
    );
    expect(localStorage.getItem("cc-launch-agent-first-scan-complete")).toBe(
      "1",
    );
  }, 15_000);

  it("opens the agent installer after a successful first scan", async () => {
    agentInstallMocks.getStatuses.mockResolvedValueOnce([
      {
        id: "codex",
        name: "Codex",
