...
- 支持在会话管理中将本地 Agent 会话转换并写回其他工具的原生会话存储。
...
在会话管理中选中源会话后，可以选择目标工具和工作目录，直接将会话转换为目标工具的原生格式并写回目标存储；目标工具支持结构化恢复时，可继续从界面打开目标会话。整个流程不要求手抄 Session ID，也不以命令行复制摘要作为主路径。

当前基于 CASR 接入 17 个 provider：Claude Code、Codex、Gemini CLI、Antigravity、Cursor、Cline、Aider、Amp、OpenCode、ChatGPT、ClawdBot、Vibe、Factory、OpenClaw、Pi-Agent、Kiro CLI 和 Grok Build。

能力边界需要注意：

- 15 个 provider 通常支持写回目标；转换会经过 canonical IR、目标原生写入和读回校验。
- OpenCode 为条件写入，受本地数据库 schema 版本限制；不兼容的 1.x/2.x 布局会在写入前拒绝。
- Antigravity 为只读目标，不能写入。
- 跨工具转换可能丢失隐藏推理、工具细节、模型元数据或目标格式不支持的附件，界面会标记“有损转换”。
- 默认不会覆盖已有目标会话；发生冲突时，用户可以显式选择“覆盖并重试”，已有目标会保留备份。
- 目标 CLI 未安装、无法自动启动或没有结构化 resume 命令时，写回结果仍会保留，并显示手动恢复提示。

详细 provider 能力、限制和验证范围见 [会话写回支持矩阵](docs/cursor/session-transfer-support.md)。
...
7. 在会话管理中选中会话，使用“转换并写回”选择目标工具和工作目录；确认写回成功后，可直接打开目标会话。
8. 如修改了命令行工具的配置，重新打开对应终端或 Agent。
